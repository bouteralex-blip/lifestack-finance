...
// 16 Tabs | 17 Upgrades Integrated | Teal-Navy Accent
...
// Radar data — factor rotation scores (P4)
const FACTOR_RADAR=[
  {subject:"Value",score:8.5,prev:4.2},{subject:"Quality",score:3.2,prev:7.8},
  {subject:"Momentum",score:5.1,prev:5.9},{subject:"Size",score:6.2,prev:3.5},
  {subject:"Growth",score:2.8,prev:8.1},{subject:"Low Vol",score:4.5,prev:6.0},
];

// Scatter data — cross-asset risk/return (P12)
const RR_SCATTER=[
  {name:"Cash",vol:0.5,ret:4.3,c:"#6DA5C0"},{name:"Gilts",vol:8,ret:-5,c:"#f43f5e"},
  {name:"IG Credit",vol:5,ret:2.1,c:"#7C6FFF"},{name:"HY Credit",vol:12,ret:3.2,c:"#f59e0b"},
  {name:"FTSE 100",vol:9.4,ret:19,c:"#0F969C"},{name:"S&P 500",vol:18,ret:-0.01,c:"#3B9EFF"},
  {name:"MSCI EM",vol:22,ret:33.6,c:"#22c55e"},{name:"Gold",vol:14,ret:80,c:"#f59e0b"},
  {name:"Bitcoin",vol:50,ret:-45.9,c:"#f7931a"},
];

// Stacked area data — credit flows over time (P2)
const CREDIT_FLOWS=[
  {m:"Sep 25",ig:1.2,hy:-0.4,em:2.1},{m:"Oct",ig:0.8,hy:-1.1,em:1.4},
  {m:"Nov",ig:1.5,hy:0.2,em:3.2},{m:"Dec",ig:2.1,hy:-0.8,em:2.8},
  {m:"Jan 26",ig:3.4,hy:1.2,em:4.1},{m:"Feb",ig:1.8,hy:-2.1,em:1.9},
  {m:"Mar",ig:0.9,hy:-0.6,em:0.5},
];
...
<div style={{height:220}}><ResponsiveContainer><LineChart data={YIELD_CURVE}><CartesianGrid strokeDasharray="4 4" stroke={T.grid}/><XAxis dataKey="t" tick={{fill:T.t3,fontSize:10}} stroke={T.grid}/><YAxis tick={{fill:T.t3,fontSize:10}} stroke={T.grid} domain={[3.4,5.0]} tickFormatter={v=>`${v}%`}/><Tooltip content={<Tip/>}/><Line type="monotone" dataKey="us" stroke={T.blue} strokeWidth={2.5} dot={{fill:T.blue,r:3}} name="US Treasury"/><Line type="monotone" dataKey="uk" stroke={T.coral} strokeWidth={2.5} dot={{fill:T.coral,r:3}} name="UK Gilt"/></LineChart></ResponsiveContainer></div>
<Ins type="insight" text={`UK curve is bear-steepening dangerously. Short end anchored (3M 3.72%, 2Y 3.80%) while 10Y+30Y explodes higher — this is a term premium shock, not a growth signal. Every 100bp move on UK 10Y gilts costs ~8-9% on long-dated positions (TLT proxy). The gap between US 10Y (4.30%) and UK 10Y (4.62%) is closing — unusual when UK growth is weaker. Iran disruption is the driver via energy-inflation expectations.`}/>
<SourceTag sources={["FRED DGS10","BoE SONIA","DMO gilt data"]}/>
</Glass>
<Glass>
<PH title="LIQUIDITY DIVERGENCE — M2 vs S&P Growth %" ac={P.cyan}/>
<div style={{height:200}}><ResponsiveContainer><ComposedChart data={LIQ_DIV}><CartesianGrid strokeDasharray="4 4" stroke={T.grid}/><XAxis dataKey="m" tick={{fill:T.t3,fontSize:10}} stroke={T.grid}/><YAxis tick={{fill:T.t3,fontSize:10}} stroke={T.grid} tickFormatter={v=>`${v}%`}/><Tooltip content={<Tip/>}/><Bar dataKey="div" name="Divergence %" radius={[4,4,0,0]}>{LIQ_DIV.map((e,i)=><Cell key={i} fill={e.div>0?T.teal:T.coral}/>)}</Bar><ReferenceLine y={5} stroke={T.teal} strokeDasharray="6 3" strokeOpacity={0.5} label={{value:"Bullish >5%",fill:T.teal,fontSize:9}}/><ReferenceLine y={-5} stroke={T.coral} strokeDasharray="6 3" strokeOpacity={0.5} label={{value:"Bearish <-5%",fill:T.coral,fontSize:9}}/><ReferenceLine y={0} stroke={T.t3} strokeOpacity={0.3}/></ComposedChart></ResponsiveContainer></div>
...
<div style={{height:200}}><ResponsiveContainer><LineChart data={CREDIT_TL}><CartesianGrid strokeDasharray="4 4" stroke={T.grid}/><XAxis dataKey="d" tick={{fill:T.t3,fontSize:10}} stroke={T.grid}/><YAxis tick={{fill:T.t3,fontSize:10}} stroke={T.grid}/><Tooltip content={<Tip/>}/><Line type="monotone" dataKey="ig" stroke={T.blue} strokeWidth={2} name="IG OAS"/><Line type="monotone" dataKey="hy" stroke={T.coral} strokeWidth={2} name="HY OAS"/><ReferenceLine y={400} stroke={T.negative} strokeDasharray="8 4" strokeOpacity={0.4}/></LineChart></ResponsiveContainer></div>
...
<Glass>
<PH title="CROSS-ASSET FUND FLOWS — IG vs HY vs EM (rolling monthly $B)" ac={P.cyan}/>
<div style={{height:190}}><ResponsiveContainer><AreaChart data={CREDIT_FLOWS}><CartesianGrid strokeDasharray="4 4" stroke={T.grid}/><XAxis dataKey="m" tick={{fill:T.t3,fontSize:10}} stroke={T.grid}/><YAxis tick={{fill:T.t3,fontSize:10}} stroke={T.grid} tickFormatter={v=>`$${v}B`}/><Tooltip content={<Tip/>}/><defs><linearGradient id="igGrad" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor={T.blue} stopOpacity={0.5}/><stop offset="95%" stopColor={T.blue} stopOpacity={0.05}/></linearGradient><linearGradient id="hyGrad" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor={T.coral} stopOpacity={0.5}/><stop offset="95%" stopColor={T.coral} stopOpacity={0.05}/></linearGradient><linearGradient id="emGrad" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor={T.teal} stopOpacity={0.5}/><stop offset="95%" stopColor={T.teal} stopOpacity={0.05}/></linearGradient></defs><Area type="monotone" dataKey="ig" stroke={T.blue} strokeWidth={2} fill="url(#igGrad)" name="IG Flows ($B)"/><Area type="monotone" dataKey="hy" stroke={T.coral} strokeWidth={2} fill="url(#hyGrad)" name="HY Flows ($B)"/><Area type="monotone" dataKey="em" stroke={T.teal} strokeWidth={2} fill="url(#emGrad)" name="EM Flows ($B)"/><ReferenceLine y={0} stroke={T.t3} strokeOpacity={0.4}/></AreaChart></ResponsiveContainer></div>
<Ins type="insight" text={`EM flows ($4.1B January peak) are the standout vs HY (net outflow trend). IG flows recovering but tepid — gilt shock spooked investment-grade buyers. The flow divergence confirms: smart money is rotating to EM for yield + growth, not taking HY credit risk in an inflation-scare environment.`}/>
</Glass>
...
// =========================================================================
const P3=()=>{
const np = MKTENG.narrativePulse;
const ps = MKTENG.policySurprise;
return(<div>
...
<Row style={{marginBottom:0}}>
  <MatStat label="Narrative Signal" value={np?.dominantNarrative||"Iran Shock"} sub={`Velocity ${np?.velocityScore?.toFixed(0)||'?'}/100 · ${np?.topThemes?.length||0} active themes`} mat={np?.velocityScore>60?"red":"amber"}/>
  <MatStat label="Policy Surprise" value={ps?.surpriseLevel||"ELEVATED"} sub={`Score ${ps?.compositeScore?.toFixed(0)||'?'}/100 · ${ps?.lastShock||'BoE cut <20%'}`} mat={ps?.compositeScore>50?"red":"amber"}/>
...
{/* Engine-driven Narrative Pulse panel */}
{np && (<Glass><PH title="NARRATIVE PULSE ENGINE — LIVE ANALYSIS" ac={P.s2}/>
<MetricGrid items={[
  {l:"Dominant Narrative",v:np.dominantNarrative||'—',n:np.narrativeShift||'Shifting',c:T.amber},
  {l:"Velocity Score",v:`${np.velocityScore?.toFixed(0)||0}/100`,n:np.velocityScore>60?'HIGH velocity — market-moving':'Moderate velocity',c:np.velocityScore>60?T.coral:T.teal},
  {l:"Consensus Reading",v:np.consensusLevel||'—',n:`${np.contrarian?'Contrarian signal active':'Consensus aligned'}`,c:np.contrarian?T.amber:T.neutral},
  {l:"Theme Count",v:`${np.topThemes?.length||0} active`,n:np.topThemes?.slice(0,2).join(', ')||'—',c:T.cyan},
]}/>
{np.topThemes?.length>0&&<div style={{marginTop:8}}>{np.topThemes.slice(0,5).map((t,i)=>(
  <div key={i} style={{padding:"6px 10px",marginBottom:4,background:"rgba(7,46,51,0.45)",borderRadius:8,borderLeft:`3px solid ${i<2?T.coral:T.amber}`,fontSize:11,color:T.t2}}>
    <span style={{color:T.cyan,fontWeight:700}}>{i+1}.</span> {typeof t==='string'?t:t.theme||t.name||JSON.stringify(t)}
  </div>))}</div>}
</Glass>)}

{/* Engine-driven Policy Surprise panel */}
{ps && (<Glass><PH title="POLICY SURPRISE DETECTOR" ac={P.s4}/>
<MetricGrid items={[
  {l:"Surprise Level",v:ps.surpriseLevel||'—',c:ps.compositeScore>50?T.coral:T.teal},
  {l:"Composite Score",v:`${ps.compositeScore?.toFixed(0)||0}/100`,n:ps.compositeScore>60?'HIGH — policy shock active':'Within expectations',c:ps.compositeScore>60?T.negative:T.neutral},
  {l:"Last Shock",v:ps.lastShock||'—',c:T.amber},
  {l:"Market Impact",v:ps.marketImpact||'—',c:T.cyan},
]}/>
<Ins type="warning" text={ps.narrative||`Policy surprise score at ${ps.compositeScore?.toFixed(0)||'?'}/100. ${ps.surpriseLevel==='HIGH'?'Major policy divergence detected — adjust positioning for rate path repricing.':'Policy path tracking within expectations. Monitor next central bank meeting.'}`}/>
</Glass>)}
...
<div style={{height:230}}><ResponsiveContainer><BarChart data={REGION_RET} layout="vertical" margin={{left:85}}><CartesianGrid strokeDasharray="4 4" stroke={T.grid} horizontal={false}/><XAxis type="number" tick={{fill:T.t3,fontSize:10}} stroke={T.grid} tickFormatter={v=>`${v}%`}/><YAxis type="category" dataKey="r" tick={{fill:T.t2,fontSize:10}} stroke={T.grid} width={80}/><Tooltip content={<Tip/>}/><Bar dataKey="v" name="12M %" radius={[0,6,6,0]}>{REGION_RET.map((e,i)=><Cell key={i} fill={e.c}/>)}</Bar></BarChart></ResponsiveContainer></div></Glass>

<Glass><PH title="SECTOR PERFORMANCE — 12-MONTH"/>
<div style={{height:230}}><ResponsiveContainer><BarChart data={SECTOR} layout="vertical" margin={{left:95}}><CartesianGrid strokeDasharray="4 4" stroke={T.grid} horizontal={false}/><XAxis type="number" tick={{fill:T.t3,fontSize:10}} stroke={T.grid} tickFormatter={v=>`${v}%`}/><YAxis type="category" dataKey="s" tick={{fill:T.t2,fontSize:10}} stroke={T.grid} width={90}/><Tooltip content={<Tip/>}/><Bar dataKey="v" name="12M %" radius={[0,6,6,0]}>{SECTOR.map((e,i)=><Cell key={i} fill={e.c}/>)}</Bar></BarChart></ResponsiveContainer></div></Glass>

<Grid cols="1fr 1fr">
<Glass><PH title="FACTOR LEADERSHIP — THE VALUE ROTATION"/>
<Tbl h={["Factor","CY2025","YTD 2026","Status"]} r={FACTORS.map(f=>[f.f,`+${f.r}%`,f.y?`${f.y>0?"+":""}${f.y}%`:"N/A",f.st])} hl={1}/>
<Ins type="insight" text={`Value vs Growth spread hit 99.8th percentile — second widest weekly spread since 2000. Enhanced Value (+39% CY2025) vs pure Growth (+19%) gap is structural: rates regime, energy/defence/banks, and mean-reversion from 2020-21 growth bubble. Not a trade — a regime shift.`}/>
</Glass>
<Glass><PH title="FACTOR ROTATION RADAR — CURRENT vs PRIOR QUARTER" ac={P.s2}/>
<div style={{height:230}}><ResponsiveContainer><RadarChart data={FACTOR_RADAR} cx="50%" cy="50%" outerRadius="70%"><PolarGrid stroke={T.grid}/><PolarAngleAxis dataKey="subject" tick={{fill:T.t3,fontSize:10}}/><PolarRadiusAxis angle={30} domain={[0,10]} tick={{fill:T.t3,fontSize:8}} tickCount={3}/><Tooltip content={<Tip/>}/><Radar name="Current" dataKey="score" stroke={P.cyan} fill={P.cyan} fillOpacity={0.25} strokeWidth={2}/><Radar name="Prev Qtr" dataKey="prev" stroke={P.s2} fill={P.s2} fillOpacity={0.12} strokeWidth={1.5} strokeDasharray="5 3"/><Legend wrapperStyle={{fontSize:10,color:P.t3}}/></RadarChart></ResponsiveContainer></div>
<Ins type="regime" text={`Value & Size have surged (8.5 & 6.2/10) while Growth & Quality have collapsed (2.8 & 3.2/10). This is the most extreme factor rotation since 2000. Momentum holding neutral — not yet chasing value or confirming growth recovery.`}/>
</Glass>
</Grid>
...
<div style={{height:220}}><ResponsiveContainer><LineChart data={YIELD_CURVE}><CartesianGrid strokeDasharray="4 4" stroke={T.grid}/><XAxis dataKey="t" tick={{fill:T.t3,fontSize:10}} stroke={T.grid}/><YAxis tick={{fill:T.t3,fontSize:10}} stroke={T.grid} domain={[3.4,5.0]} tickFormatter={v=>`${v}%`}/><Tooltip content={<Tip/>}/><Line type="monotone" dataKey="us" stroke={T.blue} strokeWidth={2.5} dot={{fill:T.blue,r:3}} name="UST"/><Line type="monotone" dataKey="uk" stroke={T.coral} strokeWidth={2.5} dot={{fill:T.coral,r:3}} name="Gilt"/></LineChart></ResponsiveContainer></div>
...
<Row style={{marginBottom:0}}>
  <MatStat label="DXY Regime" value={fx?.dollarRegime||M.dxy.toString()} sub={`${fx?.dollarBias||'Weak'} · Score ${fx?.compositeScore?.toFixed(0)||'?'}/100`} mat={fx?.dollarBias==='Strong'?"red":"teal"}/>
  ...
  <MatStat label="EM Signal" value={fx?.emSignal||"BULLISH"} sub={fx?.emRationale||"$4B inflows Jan · rotation confirmed"} mat="dark"/>
</Row>
...
{/* FX Regime Engine output */}
{fx && (<Glass><PH title="FX REGIME ENGINE — LIVE ANALYSIS" ac={P.s2}/>
<MetricGrid items={[
  {l:"Dollar Regime",v:fx.dollarRegime||'—',c:fx.dollarBias==='Weak'?T.teal:T.coral},
  {l:"Composite Score",v:`${fx.compositeScore?.toFixed(0)||0}/100`,c:T.cyan},
  {l:"Carry Attractiveness",v:fx.carryScore||'—',c:T.teal},
  {l:"EM FX Bias",v:fx.emSignal||'—',c:fx.emSignal==='BULLISH'?T.teal:T.amber},
  {l:"Volatility Regime",v:fx.fxVolRegime||'—',c:T.neutral},
  {l:"Key Risk",v:fx.keyRisk||'—',c:T.coral},
]}/>
{fx.narrative&&<Ins type="insight" text={fx.narrative}/>}
</Glass>)}
...
// =========================================================================
const P7=()=>{
const cs = MKTENG.commodityShock;
return(<div>
...
<Row style={{marginBottom:0}}>
  <MatStat label="Commodity Stress" value={cs?.shockLevel||"ELEVATED"} sub={`Score ${cs?.compositeScore?.toFixed(0)||'?'}/100 · ${cs?.topShock||'Energy'} leading`} mat={cs?.compositeScore>60?"red":"amber"}/>
  ...
  <MatStat label="Supply Signal" value={cs?.supplySignal||"DISRUPTED"} sub={cs?.supplyNarrative||"Hormuz risk elevated"} mat={cs?.supplySignal==='NORMAL'?"teal":"red"}/>
</Row>
...
{/* Engine-driven Commodity Shock analysis */}
{cs && (<Glass><PH title="COMMODITY SHOCK ENGINE — LIVE ANALYSIS" ac={P.amber}/>
<MetricGrid items={[
  {l:"Shock Level",v:cs.shockLevel||'—',c:cs.compositeScore>50?T.coral:T.teal},
  {l:"Composite Score",v:`${cs.compositeScore?.toFixed(0)||0}/100`,c:cs.compositeScore>60?T.negative:T.neutral},
  {l:"Top Shock",v:cs.topShock||'—',c:T.amber},
  {l:"Supply Signal",v:cs.supplySignal||'—',c:cs.supplySignal==='DISRUPTED'?T.coral:T.teal},
  {l:"Inflation Impact",v:cs.inflationImpact||'—',c:T.coral},
  {l:"Portfolio Action",v:cs.portfolioAction||'—',c:T.cyan},
]}/>
{cs.narrative&&<Ins type="warning" text={cs.narrative}/>}
{cs.shocks?.length>0&&<div style={{marginTop:8}}>{cs.shocks.slice(0,4).map((s,i)=>(
  <div key={i} style={{padding:"6px 10px",marginBottom:4,background:"rgba(7,46,51,0.45)",borderRadius:8,borderLeft:`3px solid ${s.severity==='HIGH'?T.coral:T.amber}`,fontSize:11,color:T.t2}}>
    <span style={{fontWeight:700,color:T.t1}}>{s.commodity||s.name||'—'}</span> — {s.signal||s.description||'Monitoring'}
  </div>))}</div>}
</Glass>)}
...
// =========================================================================
const P8=()=>{
const pc = MKTENG.propertyCycle || (typeof computePropertyCycleState==='function'?computePropertyCycleState(M):null);
return(<div>
<Hd t="HOUSING, PROPERTY & REAL ESTATE" s="Affordability, mortgage rates, segment split, REITs" tag="PROPERTY" showFreshness/>
<Row style={{marginBottom:0}}>
  <MatStat label="Property Cycle" value={pc?.phase||"SLOWDOWN"} sub={`Score ${pc?.score?.toFixed(0)||'?'}/100 · ${pc?.verdict||'Caution warranted'}`} mat={pc?.phase==='EXPANSION'?"teal":pc?.phase==='DOWNTURN'?"red":"amber"}/>
  <MatStat label="Mortgage Rate" value={pc?.mortgageRate?`${pc.mortgageRate}%`:"~4.5%+"} sub="Rising on gilt surge · caution" mat="red"/>
  <MatStat label="HPI Signal" value={pc?.hpi||"+1.5–3.5%"} sub={pc?.implications?.[0]||"Gilt shock threatens recovery"} mat="amber"/>
...
{/* Property Cycle Engine output */}
{pc && (<Glass><PH title="PROPERTY CYCLE ENGINE — LIVE ANALYSIS" ac={P.amber}/>
<MetricGrid items={[
  {l:"Cycle Phase",v:pc.phase||'—',c:pc.phase==='EXPANSION'?T.teal:pc.phase==='DOWNTURN'?T.negative:T.amber},
  {l:"Cycle Score",v:`${pc.score?.toFixed(0)||0}/100`,c:T.cyan},
  {l:"Mortgage Rate",v:pc.mortgageRate?`${pc.mortgageRate}%`:'—',c:T.coral},
  {l:"HPI",v:pc.hpi||'—',c:T.neutral},
]}/>
{pc.signals?.length>0&&<div style={{marginTop:8}}>{pc.signals.slice(0,4).map((s,i)=>(
  <div key={i} style={{padding:"5px 10px",marginBottom:3,background:"rgba(7,46,51,0.45)",borderRadius:8,borderLeft:`3px solid ${T.amber}`,fontSize:11,color:T.t2}}>{typeof s==='string'?s:s.signal||JSON.stringify(s)}</div>
))}</div>}
{pc.implications?.length>0&&<Ins type="insight" text={pc.implications.join(' ')}/>}
</Glass>)}
...
<div style={{height:230}}><ResponsiveContainer><ComposedChart data={CRYPTO_TL}><CartesianGrid strokeDasharray="4 4" stroke={T.grid}/><XAxis dataKey="d" tick={{fill:T.t3,fontSize:10}} stroke={T.grid}/><YAxis yAxisId="l" tick={{fill:T.t3,fontSize:10}} stroke={T.grid} tickFormatter={v=>`$${v}K`}/><YAxis yAxisId="r" orientation="right" tick={{fill:T.t3,fontSize:10}} stroke={T.grid} domain={[0,100]}/><Tooltip content={<Tip/>}/><defs><linearGradient id="bg" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor={T.btc} stopOpacity={0.3}/><stop offset="100%" stopColor={T.btc} stopOpacity={0.02}/></linearGradient></defs><Area yAxisId="l" type="monotone" dataKey="btc" fill="url(#bg)" stroke={T.btc} strokeWidth={2.5} name="BTC ($K)"/><Line yAxisId="r" type="monotone" dataKey="fg" stroke={T.coral} strokeWidth={1.5} strokeDasharray="4 3" dot={false} name="Fear & Greed"/><ReferenceLine yAxisId="r" y={25} stroke={T.coral} strokeDasharray="8 4" strokeOpacity={0.4}/></ComposedChart></ResponsiveContainer></div></Glass>
...
// =========================================================================
const P10=()=>{
const cftc = MKTENG.cftcPositioning;
const ef = MKTENG.etfFlows;
return(<div>
...
<Row style={{marginBottom:0}}>
  <MatStat label="Flow Signal" value={ef?.overallSignal||"DIVERGING"} sub={`${ef?.topFlow||'EM inflows'} · ${ef?.opportunities?.[0]?.theme||'Rotation active'}`} mat={ef?.overallSignal==='BULLISH'?"teal":"amber"}/>
  <MatStat label="CFTC Crowding" value={cftc?.crowdingLevel||"ELEVATED"} sub={`Score ${cftc?.compositeScore?.toFixed(0)||'?'}/100 · ${cftc?.mostCrowded||'Oil longs'}`} mat={cftc?.compositeScore>60?"red":"amber"}/>
...
{/* Engine-driven CFTC Positioning panel */}
{cftc && (<Glass><PH title="CFTC POSITIONING ENGINE — CROWDING ANALYSIS" ac={P.s4}/>
<MetricGrid items={[
  {l:"Crowding Level",v:cftc.crowdingLevel||'—',c:cftc.compositeScore>50?T.coral:T.teal},
  {l:"Most Crowded",v:cftc.mostCrowded||'—',c:T.coral},
  {l:"Contrarian Signal",v:cftc.contrarianSignal||'—',c:T.amber},
  {l:"Net Spec Position",v:cftc.netSpeculative||'—',c:T.neutral},
]}/>
{cftc.narrative&&<Ins type="warning" text={cftc.narrative}/>}
</Glass>)}
...
// =========================================================================
const P11=()=>{
const fr = MKTENG.factorRotation;
const er = MKTENG.earningsRevision;
const cd = MKTENG.correlationDrift;
const gr = MKTENG.gapRisk;
return(<div>
...
<Row style={{marginBottom:0}}>
  <MatStat label="Factor Signal" value={fr?.dominantFactor||"VALUE"} sub={`Rotation ${fr?.rotationStrength||'EXTREME'} · Score ${fr?.compositeScore?.toFixed(0)||'?'}/100`} mat={fr?.rotationStrength==='EXTREME'?"amber":"teal"}/>
  <MatStat label="Earnings Rev" value={er?.revisionDirection||"MIXED"} sub={`Breadth ${er?.breadthScore?.toFixed(0)||'?'}% · ${er?.topRevision||'EM improving'}`} mat={er?.revisionDirection==='POSITIVE'?"teal":"amber"}/>
  <MatStat label="MSCI EM PE" value="~12x" sub="20-year discount to DM · best R/R" mat="indigo"/>
  <MatStat label="Gap Risk" value={gr?.riskLevel||"MODERATE"} sub={`Score ${gr?.compositeScore?.toFixed(0)||'?'}/100 · ${gr?.nextEvent||'Watching calendar'}`} mat={gr?.compositeScore>60?"red":"amber"}/>
</Row>
...
{/* Engine-driven Factor Rotation panel */}
{fr && (<Glass><PH title="FACTOR ROTATION ENGINE" ac={P.s2}/>
<MetricGrid items={[
  {l:"Dominant Factor",v:fr.dominantFactor||'—',c:T.teal},
  {l:"Rotation Strength",v:fr.rotationStrength||'—',c:fr.rotationStrength==='EXTREME'?T.coral:T.amber},
  {l:"Value Score",v:`${fr.valueScore?.toFixed(1)||'?'}/10`,c:T.teal},
  {l:"Growth Score",v:`${fr.growthScore?.toFixed(1)||'?'}/10`,c:T.coral},
  {l:"Quality Score",v:`${fr.qualityScore?.toFixed(1)||'?'}/10`,c:T.neutral},
  {l:"Momentum Score",v:`${fr.momentumScore?.toFixed(1)||'?'}/10`,c:T.amber},
]}/>
{fr.narrative&&<Ins type="regime" text={fr.narrative}/>}
</Glass>)}

{/* Engine-driven Earnings Revision panel */}
{er && (<Glass><PH title="EARNINGS REVISION MONITOR" ac={P.cyan}/>
<MetricGrid items={[
  {l:"Revision Direction",v:er.revisionDirection||'—',c:er.revisionDirection==='POSITIVE'?T.teal:T.coral},
  {l:"Breadth Score",v:`${er.breadthScore?.toFixed(0)||0}%`,c:er.breadthScore>50?T.teal:T.coral},
  {l:"Top Revision",v:er.topRevision||'—',c:T.teal},
  {l:"Worst Revision",v:er.worstRevision||'—',c:T.coral},
]}/>
{er.narrative&&<Ins type="insight" text={er.narrative}/>}
</Glass>)}

<Grid cols="1fr 1fr">
{/* Correlation Drift panel */}
{cd && (<Glass><PH title="CORRELATION DRIFT MONITOR" ac={P.s4}/>
<MetricGrid items={[
  {l:"Regime",v:cd.correlationRegime||'—',c:cd.correlationRegime==='HIGH'?T.coral:T.teal},
  {l:"Avg Correlation",v:cd.avgCorrelation?.toFixed(2)||'—',c:T.neutral},
  {l:"Diversification",v:cd.diversificationScore||'—',c:cd.diversificationScore==='POOR'?T.coral:T.teal},
]}/>
{cd.narrative&&<Ins type="warning" text={cd.narrative}/>}
</Glass>)}

{/* Gap Risk panel */}
{gr && (<Glass><PH title="GAP RISK DETECTOR" ac={P.s4}/>
<MetricGrid items={[
  {l:"Risk Level",v:gr.riskLevel||'—',c:gr.compositeScore>50?T.coral:T.teal},
  {l:"Next Event",v:gr.nextEvent||'—',c:T.amber},
  {l:"Vol Premium",v:gr.volPremium||'—',c:T.neutral},
]}/>
{gr.narrative&&<Ins type="risk" text={gr.narrative}/>}
</Glass>)}
</Grid>
...
<div style={{height:210}}><ResponsiveContainer><ComposedChart data={VOL_TL}><CartesianGrid strokeDasharray="4 4" stroke={T.grid}/><XAxis dataKey="d" tick={{fill:T.t3,fontSize:10}} stroke={T.grid}/><YAxis tick={{fill:T.t3,fontSize:10}} stroke={T.grid}/><Tooltip content={<Tip/>}/><Line type="monotone" dataKey="vix" stroke={T.blue} strokeWidth={2} name="VIX"/><Line type="monotone" dataKey="move" stroke={T.coral} strokeWidth={2} name="MOVE" strokeDasharray="6 3"/><Line type="monotone" dataKey="cv" stroke={T.btc} strokeWidth={2} name="Crypto Vol"/><ReferenceLine y={20} stroke={T.amber} strokeDasharray="6 4" strokeOpacity={0.4}/></ComposedChart></ResponsiveContainer></div>
...
<SourceTag sources={["FRED VIXCLS","MacroMicro MOVE","FRED STLFSI4/NFCI","Phase 2: Riskfolio-Lib, skfolio"]}/>
</Glass>
<Glass><PH title="RISK/RETURN SCATTER — 12-MONTH CROSS-ASSET" ac={P.s4}/>
<div style={{height:230}}><ResponsiveContainer><ScatterChart margin={{top:10,right:20,bottom:20,left:10}}>
<CartesianGrid strokeDasharray="4 4" stroke={T.grid}/>
<XAxis type="number" dataKey="vol" name="Volatility %" tick={{fill:T.t3,fontSize:9}} stroke={T.grid} label={{value:"Volatility %",position:"insideBottom",offset:-8,fill:T.t3,fontSize:9}}/>
<YAxis type="number" dataKey="ret" name="12M Return %" tick={{fill:T.t3,fontSize:9}} stroke={T.grid} label={{value:"12M Return %",angle:-90,position:"insideLeft",offset:10,fill:T.t3,fontSize:9}}/>
<Tooltip cursor={{strokeDasharray:"3 3",stroke:T.t3}} content={({active,payload})=>{
  if(!active||!payload?.length)return null;
  const d=payload[0]?.payload;
  return<div style={{background:"rgba(5,22,26,0.92)",border:`1px solid ${P.b1}`,borderRadius:8,padding:"6px 10px",fontSize:11}}>
    <div style={{color:P.t1,fontWeight:700}}>{d?.name}</div>
    <div style={{color:P.t3}}>Vol: {d?.vol}% · Ret: {d?.ret>0?"+":""}{d?.ret}%</div>
  </div>;
}}/>
<ReferenceLine y={0} stroke={T.t3} strokeOpacity={0.4}/>
<ReferenceLine x={4.3} stroke={T.blue} strokeDasharray="6 4" strokeOpacity={0.5} label={{value:"Cash hurdle",fill:T.blue,fontSize:8}}/>
<Scatter data={RR_SCATTER} shape={(props)=>{
  const{cx,cy,payload}=props;
  return<g><circle cx={cx} cy={cy} r={6} fill={payload.c} opacity={0.85} stroke="rgba(255,255,255,0.2)" strokeWidth={1}/><text x={cx} y={cy-9} textAnchor="middle" fill={P.t3} fontSize={8}>{payload.name}</text></g>;
}}/>
</ScatterChart></ResponsiveContainer></div>
<Ins type="insight" text={`Gold is the outlier: +80% return with only 14% vol — best risk-adjusted asset of 2026. Bitcoin is the opposite extreme: 50% vol for -46% return. Cash at 0.5% vol/4.3% return sits at the left edge — your true risk-free rate. Everything must clear this hurdle.`}/>
</Glass>
</Grid>
...
<div style={{fontSize:10.5,color:T.t2,marginTop:3}}>{x.desc}</div></div>))}
</Glass>
<Glass><PH title="PROBABILITY DISTRIBUTION — SCENARIO WEIGHTS" ac={P.s2}/>
<div style={{height:200,display:"flex",alignItems:"center",justifyContent:"center"}}><ResponsiveContainer width="100%" height={190}><PieChart>
<defs>
  {SCENARIOS.map((s,i)=><linearGradient key={i} id={`sg${i}`} x1="0" y1="0" x2="1" y2="1"><stop offset="0%" stopColor={s.col} stopOpacity={0.9}/><stop offset="100%" stopColor={s.col} stopOpacity={0.55}/></linearGradient>)}
</defs>
<Pie data={SCENARIOS.map((s,i)=>({name:s.s,value:s.pr,col:s.col}))} cx="50%" cy="50%" innerRadius={52} outerRadius={78} paddingAngle={3} dataKey="value" label={({name,value})=>`${value}%`} labelLine={false}>
  {SCENARIOS.map((s,i)=><Cell key={i} fill={`url(#sg${i})`} stroke={s.col} strokeWidth={1}/>)}
</Pie>
<Tooltip formatter={(value,name)=>[`${value}%`,name]} contentStyle={{background:"rgba(5,22,26,0.92)",border:`1px solid ${P.b1}`,borderRadius:8,fontSize:11}}/>
<Legend iconSize={8} wrapperStyle={{fontSize:9,color:P.t3}}/>
</PieChart></ResponsiveContainer></div>
<Ins type="warning" text={`Downside probability (Bear + Crisis) = 30%. Combined portfolio impact at -30% probability = -7.3% expected loss from these tails. Iran escalation moved Bear scenario from 15% → 20%. Monitor Brent >$100 — that upgrades Bear to 25% and shifts the probability centroid away from Base.`}/>
</Glass>
</Grid>
...
// =========================================================================
const P16=()=>{
const reg = MKTENG.regime;
const stress = MKTENG.stress;
const btc = MKTENG.btcCycle;
const yc = MKTENG.yieldCurve;
const credit = MKTENG.creditStress;
const sl = MKTENG.sectorLeadership;
const ef = MKTENG.etfFlows;
const co = MKTENG.cryptoOnChain;
const stressColor = stress?.compositeScore > 60 ? "red" : stress?.compositeScore > 30 ? "amber" : "teal";
return (<div>
<Hd t="WEEKLY INTELLIGENCE SYNTHESIS" s={`Week of ${M.date}`} tag="ENGINE-DRIVEN" showFreshness/>

{/* Hero tiles — engine-driven regime + stress + BTC cycle */}
<Row style={{marginBottom:0}}>
  <MatStat label="Regime" value={reg?.regime || "LATE CYCLE"} sub={`${reg?.riskPosture || 'Cautious'} \u00B7 ${reg?.confidence || 68}% conf`} mat={reg?.riskPosture === 'Max Defensive' ? "red" : reg?.riskPosture === 'Defensive' ? "amber" : "indigo"}/>
  <MatStat label="Market Stress" value={`${stress?.compositeScore || 0}/100`} sub={`${stress?.compositeLevel || 'Normal'} \u00B7 ${stress?.topStressors?.[0]?.asset || 'VIX'} elevated`} mat={stressColor}/>
  <MatStat label="BTC Cycle" value={btc?.phase || "ACCUMULATION"} sub={`${btc?.posture || ''} \u00B7 Bias ${btc?.bias || 0}/5`} mat={btc?.bias >= 3 ? "teal" : "amber"}/>
  <MatStat label="Credit" value={credit?.compositeLevel || "Normal"} sub={`Score ${credit?.compositeScore || 0}/100 \u00B7 ${credit?.trend || 'Stable'}`} mat={credit?.compositeScore > 50 ? "red" : "indigo"}/>
</Row>

{/* Engine-derived market context grid */}
<Glass glow><PH title="MARKET REGIME DASHBOARD"/>
<div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit, minmax(140px, 1fr))",gap:8}}>
{[
  {l:"Yield Curve",v:yc?.shape || "NORMAL",c:yc?.shape==='INVERTED'?T.coral:T.teal},
  {l:"Steepness",v:yc?.steepnessScore ? `${yc.steepnessScore.toFixed(1)}/10` : '\u2014',c:T.t2},
  {l:"Duration Bias",v:yc?.durationBias || '\u2014',c:T.amber},
  {l:"Sector Breadth",v:sl?.marketBreadth || '\u2014',c:T.t2},
  {l:"Rotation",v:sl?.rotationSignal || '\u2014',c:sl?.rotationSignal==='EXTREME'?T.coral:T.t2},
  {l:"ETF Flows",v:ef?.overallSignal || '\u2014',c:T.t2},
  {l:"BTC Dominance",v:MKTENG.btcDominance?.phase || '\u2014',c:T.btc},
  {l:"On-Chain Health",v:co?.healthLevel || '\u2014',c:co?.healthScore>60?T.teal:T.amber},
  {l:"Contagion",v:stress?.contagionRisk || 'LOW',c:stress?.contagionRisk==='HIGH'?T.coral:T.t2},
].map((m,i)=>(<div key={i} style={{padding:"8px 10px",background:"rgba(7,46,51,0.45)",borderRadius:8}}>
  <div style={{fontSize:9,color:T.t4,fontWeight:700,letterSpacing:0.5}}>{m.l}</div>
  <div style={{fontSize:13,color:m.c,fontWeight:700,marginTop:2}}>{m.v}</div>
</div>))}
</div></Glass>

{/* Stress Asset Breakdown — from engine */}
{stress?.topStressors?.length > 0 && (
<Glass><PH title="CROSS-ASSET STRESS BREAKDOWN" ac={P.s4}/>
{stress.topStressors.slice(0,6).map((s,i)=>(<div key={i} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"5px 0",borderBottom:`1px solid ${T.grid}`}}>
  <span style={{fontSize:11,color:T.t2}}>{s.asset}</span>
  <div style={{display:"flex",alignItems:"center",gap:8}}>
    <div style={{width:80,height:6,borderRadius:3,background:"rgba(7,46,51,0.45)",overflow:"hidden"}}><div style={{width:`${Math.min(s.score,100)}%`,height:"100%",borderRadius:3,background:s.score>60?T.coral:s.score>30?T.amber:T.teal}}/></div>
    <span style={{fontSize:11,fontWeight:700,color:s.score>60?T.coral:s.score>30?T.amber:T.teal,minWidth:30,textAlign:"right"}}>{s.score?.toFixed(0)}</span>
  </div>
</div>))}</Glass>
)}

<Grid cols="1fr 1fr">
<Glass><PH title="KEY RISKS" ac={P.s4}/>
{[
  stress?.compositeScore > 40 && `Market stress elevated at ${stress.compositeScore}/100. ${stress.compositeAction || 'Monitor risk exposure.'}`,
  credit?.compositeScore > 30 && `Credit spreads ${credit.compositeLevel}. HY OAS at ${M.hyOAS || '?'}bps.`,
  yc?.shape === 'INVERTED' && `Yield curve inverted \u2014 recession signal active. Duration: short.`,
  reg?.regime === 'STAGFLATION' && `Stagflation regime \u2014 high inflation + low growth. Defensive posture.`,
  co?.healthScore < 50 && `On-chain health deteriorating (${co.healthScore}/100). Risk-off crypto.`,
].filter(Boolean).concat(["Crypto-equity correlation rising. Zero diversification benefit from 13% crypto weight."]).slice(0,3)
...
<Glass><PH title="KEY OPPORTUNITIES" ac={P.cyan}/>
{[
  btc?.bias >= 2 && `BTC in ${btc.phase} \u2014 accumulation bias ${btc.bias}/5. ${btc.posture || 'Continue DCA.'}`,
  sl?.leaders?.length > 0 && `Sector leadership: ${sl.leaders.slice(0,2).join(', ')}. Breadth ${sl.breadth12m || '?'}%.`,
  ef?.opportunities?.length > 0 && `ETF flow opportunity: ${ef.opportunities[0]?.theme || 'Contrarian signal detected'}.`,
  reg?.regime === 'GOLDILOCKS' && `Goldilocks regime \u2014 risk-on. Lean into equities.`,
  co?.healthScore > 70 && `On-chain health strong (${co.healthScore}/100). Crypto allocation supported.`,
].filter(Boolean).concat(["ISA deployment: 80-120bps structural alpha. Highest certainty action."]).slice(0,3)
...
<Tbl h={["Sleeve","Engine Signal","Action"]} r={[
["JURE (US)",`${reg?.regime || 'Late Cycle'} regime. ${sl?.leaders?.includes('Info Tech') ? 'Tech leading.' : 'Value rotation headwind.'}`,reg?.riskPosture==='Defensive'?"Trim":"Hold"],
["JERE (Europe)",`Europe 12M: +${M.msciEurope12m || 36}%. ${sl?.rotationSignal || 'Rotation'} signal.`,"ADD via ISA"],
["JMRE (EM)",`EM strong. Dollar ${M.dxy < 100 ? 'weak \u2014 EM tailwind' : 'strong \u2014 EM headwind'}.`,"ADD via ISA"],
["JUKC (UK)","FTSE value tilt helps. Gilt shock headwind.","Hold"],
["BTC",`${btc?.phase || 'Accumulation'}. F&G ${M.fng || '?'}. On-chain: ${co?.healthLevel || '?'}.`,btc?.bias>=2?"Continue DCA":"Hold"],
["ETH / SOL",`BTC dominance ${MKTENG.btcDominance?.phase || '?'}. Alt risk elevated.`,MKTENG.btcDominance?.phase==='BTC SEASON'?"Do NOT add":"Monitor"],
["Cash / FD",`Best 1Y at ${M.savings1Y || 4.30}%. Real yield ~${((M.savings1Y||4.30)-(M.cpi||3.2)).toFixed(1)}%.`,"Deploy ISA. Cash above 3-mo buffer is drag."],
["Amex Debt",`\u00A3${(M.amexDebt||10652).toLocaleString()} at 22% APR.`,"CLEAR \u2014 guaranteed 22% return"]
]} hl={2}/></Glass>

<Glass style={{borderTop:`2px solid ${T.teal}`}}><PH title="ACTION SUMMARY" ac={P.cyan}/>
<div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit, minmax(190px, 1fr))",gap:10}}>
{[{l:"DO THIS WEEK",items:[
  "Deploy ISA \u00A320K: JERE + JMRE",
  btc?.bias>=2 ? "Continue BTC DCA" : "Hold BTC position",
  "Review Amex paydown (\u00A310.6K)"
],c:T.teal},
{l:"WATCH CLOSELY",items:[
  "BoE MPC 19 March",
  `Stress score (currently ${stress?.compositeScore || 0}/100)`,
  `Gilt 10Y (${yc?.shape === 'INVERTED' ? 'INVERTED \u2014 crisis' : 'Normal'})`,
  "BTC ETF daily flows"
],c:T.amber},
...
const SECS={A:"PERSONAL MARKETS & PORTFOLIO"};
  ...
  {side&&<div style={{width:256,minWidth:256,position:"sticky",top:0,height:"100vh",zIndex:10,overflow:'hidden'}}>
    ...
    <div style={{position:'relative',zIndex:1,padding:"14px 0",overflowY:"auto",height:'100%'}}>
      ...
      <div style={{padding:"8px 18px 18px",borderBottom:`1px solid ${P.b2}`}}>
        ...
        <div style={{fontSize:8.5,color:P.t3,marginTop:3}}>{M.date} · 16 tabs · 17 upgrades</div>
      </div>
      {/* Nav sections */}
