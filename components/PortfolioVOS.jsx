...
const T1=()=>{
  // \u2550\u2550\u2550 DESIGN LANGUAGE v2 \u2014 DARK BENTO GLASS \u2550\u2550\u2550
  const SANS = T.sans || "'Inter','SF Pro Display',system-ui,sans-serif";
  const MONO = T.mono || "'JetBrains Mono','SF Mono',monospace";
  const ACC = "#D4A843"; // Finance module accent (amber gold)
  const ACC_L = "#E8C872";
  const ACC_M = "#A07D2E";

  // Glass tile — dark mode, 18px radius, Bento spec
  const tile = (tier=2) => ({
    background: tier===1 ? "rgba(255,255,255,0.07)" : tier===3 ? "rgba(255,255,255,0.03)" : "rgba(255,255,255,0.05)",
    backdropFilter: "blur(24px) saturate(1.6)",
    WebkitBackdropFilter: "blur(24px) saturate(1.6)",
    border: "1px solid rgba(255,255,255," + (tier===1 ? "0.12" : tier===3 ? "0.06" : "0.08") + ")",
    borderRadius: 18,
    padding: 24,
    position: "relative",
    overflow: "hidden",
  ...
  // Floating action button (top-right corner of cards)
  const Fab = ({icon}) => (
    <div style={{position:"absolute",top:14,right:14,width:28,height:28,borderRadius:10,
      background:"rgba(255,255,255,0.06)",border:"1px solid rgba(255,255,255,0.10)",
      display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",
      transition:"background 0.15s ease"}}>
      {icon || <ChevronRight size={12} color="rgba(255,255,255,0.45)"/>}
    </div>
  );

  // Tile wrapper with hover
  const DG = ({children,span,accent,tier=2,style:sx={}}) => {
    const[h,setH]=useState(false);
    return(<div onMouseEnter={()=>setH(true)} onMouseLeave={()=>setH(false)} style={{
      ...tile(tier),
      transform:h?"translateY(-3px) scale(1.004)":"none",
      boxShadow:h?"0 12px 40px rgba(0,0,0,0.35)":"0 8px 32px rgba(0,0,0,0.25)",
      gridColumn:span?("span "+span):undefined,
      ...(accent?{borderTop:"3.5px solid "+accent}:{}),
      ...sx
    }}>
      {children}
    </div>);
  ...
  // Bento typography helpers
  const Label = ({children}) => (<div style={{fontFamily:SANS,fontSize:11,fontWeight:400,
    letterSpacing:"0.03em",color:"rgba(255,255,255,0.48)",lineHeight:1.2,marginBottom:4}}>{children}</div>);
  const Hero = ({children,color}) => (<span style={{fontFamily:MONO,fontSize:44,fontWeight:700,
    letterSpacing:"-0.02em",lineHeight:1.0,color:color||"#fff"}}>{children}</span>);
  const Unit = ({children}) => (<span style={{fontFamily:SANS,fontSize:15,fontWeight:300,
    color:"rgba(255,255,255,0.50)",marginRight:2}}>{children}</span>);
  const Sub = ({children}) => (<div style={{fontFamily:SANS,fontSize:12,fontWeight:400,
    color:"rgba(255,255,255,0.55)",marginTop:6,lineHeight:1.3}}>{children}</div>);
  const Title = ({children}) => (<div style={{fontFamily:SANS,fontSize:15,fontWeight:600,
    color:"rgba(255,255,255,0.90)",letterSpacing:0}}>{children}</div>);
  const Delta = ({v,positive}) => (<span style={{fontFamily:MONO,fontSize:11,fontWeight:600,
    color:positive?"#22C55E":"#EF4444",background:(positive?"rgba(34,197,94,0.12)":"rgba(239,68,68,0.12)"),
    padding:"2px 8px",borderRadius:6}}>{v}</span>);
  const Dot = ({c,sz=6}) => (<span style={{width:sz,height:sz,borderRadius:"50%",background:c,display:"inline-block",flexShrink:0}}/>);
  const Tag = ({t,c=ACC}) => (<span style={{fontFamily:MONO,fontSize:9,fontWeight:600,
    letterSpacing:"0.08em",textTransform:"uppercase",color:c,background:c+"18",
    padding:"3px 8px",borderRadius:6}}>{t}</span>);

  // Chart plate (inner container)
  const ChartBox = ({children,h=200}) => (<div style={{background:"rgba(255,255,255,0.03)",
    borderRadius:12,padding:"12px 8px",marginTop:12}}>{children}</div>);

  // Metric row for sidebar-style small values
  const SR = ({l,v,c}) => (<div style={{display:"flex",justifyContent:"space-between",padding:"6px 0",
    borderBottom:"1px solid rgba(255,255,255,0.06)"}}>
    <span style={{fontFamily:SANS,fontSize:12,color:"rgba(255,255,255,0.55)"}}>{l}</span>
    <span style={{fontFamily:MONO,fontSize:12,fontWeight:600,color:c||"rgba(255,255,255,0.90)"}}>{v}</span>
  </div>);

  // Badge with icon
  const Badge = ({icon,label,value,color}) => (<div style={{display:"flex",alignItems:"center",gap:8,padding:"6px 0"}}>
    ...
    <div><div style={{fontFamily:SANS,fontSize:10,color:"rgba(255,255,255,0.48)"}}>{label}</div>
    <div style={{fontFamily:MONO,fontSize:13,fontWeight:700,color:"rgba(255,255,255,0.95)"}}>{value}</div></div>
  </div>);

  // Spark line (tiny, no axes)
  const Spark = ({data,color=ACC,w=80,h=30}) => (
    <ResponsiveContainer width={w} height={h}>
      <LineChart data={data}><Line type="monotone" dataKey="v" stroke={color} strokeWidth={2} dot={false}
        style={{filter:"drop-shadow(0 0 6px "+color+"55)"}}/></LineChart>
    </ResponsiveContainer>
  );

  // Toggle group
  const Tog = ({items,a=0}) => (<div style={{display:"flex",gap:2}}>
    {items.map((t,i)=>(<span key={i} style={{fontFamily:MONO,fontSize:9,fontWeight:600,padding:"3px 8px",borderRadius:8,
      background:i===a?ACC+"25":"rgba(255,255,255,0.04)",color:i===a?ACC:"rgba(255,255,255,0.45)",
      border:"1px solid "+(i===a?ACC+"45":"rgba(255,255,255,0.08)")}}>{t}</span>))}
  </div>);

  // Severity pill
  const Pill = ({text,sev}) => {
    const bg=sev==="red"?"rgba(239,68,68,0.12)":sev==="amber"?"rgba(245,158,11,0.12)":"rgba(34,197,94,0.12)";
    const fg=sev==="red"?"#F87171":sev==="amber"?"#FBBF24":"#4ADE80";
    const bc=sev==="red"?"#EF4444":sev==="amber"?"#F59E0B":"#22C55E";
    return(<span style={{fontFamily:MONO,fontSize:9,fontWeight:600,color:fg,background:bg,
      padding:"3px 10px",borderRadius:12,borderLeft:"3px solid "+bc}}>{text}</span>);
  };

  // PillLabel for chart annotations
  const PillLabel = ({x,y,value,color="#fff"}) => {
    if(!x||!y)return null;
    const w=String(value).length*6.5+16;
    return(<g><rect x={x-w/2} y={y-15} width={w} height={18} rx={9} fill="rgba(0,0,0,0.65)" stroke={color} strokeWidth={0.5} strokeOpacity={0.35}/>
      <text x={x} y={y-3} textAnchor="middle" fill={color} fontSize={9} fontFamily={MONO} fontWeight="600">{value}</text></g>);
  };
  ...
  const allocD=[{name:"ETFs",value:27.9,color:"#0D9488"},{name:"Pension",value:23,color:"#7C6FFF"},{name:"Cash/FD",value:16.4,color:"#38BDF8"},{name:"Crypto",value:13.6,color:ACC},{name:"Investments",value:12.2,color:"#3B9EFF"},{name:"Stocks",value:3.6,color:"#EC4899"},{name:"Mixed",value:3.3,color:"#EF4444"}];
  const captureD=["Sep","Oct","Nov","Dec","Jan","Feb","Mar"].map((m,i)=>({m,up:[8,5,3,2,6,4,7][i],down:[-4,-8,-12,-6,-10,-14,-5][i]}));
  const riskRetD=[{x:4,y:8,z:60,n:"Pension",c:"#7C6FFF"},{x:8,y:-12,z:22,n:"BTC",c:ACC},{x:3,y:7,z:24,n:"JURE",c:"#0D9488"},{x:5,y:5,z:17,n:"JGEP",c:"#38BDF8"},{x:6,y:21,z:20,n:"ZAR",c:"#EC4899"},{x:2,y:4,z:46,n:"FD",c:"#34D399"}];
  ...
  const holdingsAll=[{n:"Daiwa Pension",v:"\u00A360,275",w:"23.0%",r:"+28.6%",c:"#22C55E",cont:"+4.2%"},{n:"Fixed Deposit",v:"\u00A346,000",w:"17.5%",r:"+7.4%",c:"#22C55E",cont:"+0.6%"},{n:"JURE.L",v:"\u00A323,834",w:"9.1%",r:"+8.4%",c:"#22C55E",cont:"+0.7%"},{n:"BTC",v:"\u00A321,900",w:"8.3%",r:"-44.0%",c:"#EF4444",cont:"-3.5%"},{n:"ZAR Invest",v:"\u00A320,258",w:"7.7%",r:"+21.2%",c:"#22C55E",cont:"+1.5%"},{n:"Monzo Cash",v:"\u00A311,558",w:"4.4%",r:"+11.1%",c:"#22C55E",cont:"+0.3%"}];

  // \u2550\u2550\u2550 RENDER \u2550\u2550\u2550
  return(<div style={{background:"#080810",minHeight:"100vh",padding:"24px 20px",display:"flex",flexDirection:"column",gap:16,fontFamily:SANS}}>

    {/* ZONE 1: HEADER */}
    <div style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}>
      <div style={{display:"flex",alignItems:"center",gap:12}}>
        <div style={{width:8,height:8,borderRadius:"50%",background:ACC,boxShadow:"0 0 12px "+ACC+"55"}}/>
        <span style={{fontSize:22,fontWeight:700,color:"rgba(255,255,255,0.95)",letterSpacing:"-0.02em"}}>Executive Summary</span>
        <Tag t="COMMAND CENTER"/>
      </div>
      <div style={{display:"flex",gap:8}}><Tog items={["Live","6M","1Y","Max"]} a={1}/></div>
    </div>

    {/* ZONE 2: 4x HERO KPI TILES \u2014 Family grouping with sub-metrics */}
    <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:16}}>
      {[
        {l:"NET WORTH",v:"262,622",u:"\u00A3",d:"\u25BC 11.7%",pos:false,sp:nwS,sc:"#EF4444",sub:"Peak: \u00A3297,457 \u00B7 Sep 2025",ac:ACC,
         family:[{fl:"Weekly",fv:"+\u00A34k"},{fl:"Monthly",fv:"-\u00A31k"},{fl:"6M",fv:"-\u00A335k"}]},
        {l:"6M RETURN (TWR)",v:"-11.7%",u:"",d:"\u25BC \u00A334.8k",pos:false,sp:retS,sc:"#EF4444",sub:"XIRR: -13.2% \u00B7 Bench: -4.1%",ac:"#EF4444",
         family:[{fl:"Alpha",fv:"-7.6%"},{fl:"Beta",fv:"0.87"},{fl:"Sharpe",fv:"0.42"}]},
        {l:"PEAK DRAWDOWN",v:"-16.7%",u:"",d:"Feb 2026",pos:false,sp:ddS,sc:"#EF4444",sub:"CDaR\u2098: -18.4% \u00B7 Recovery: In Progress",ac:"#EF4444",
         family:[{fl:"Duration",fv:"142d"},{fl:"Recovery",fv:"0%"},{fl:"CDaR",fv:"-18.4%"}]},
        {l:"COAST FIRE",v:"34%",u:"",d:"+2.1pp",pos:true,sp:fireS,sc:"#22C55E",sub:"Target: \u00A3750k \u00B7 Gap: \u00A3487k",ac:"#22C55E",
         family:[{fl:"Target",fv:"\u00A3750k"},{fl:"Gap",fv:"\u00A3488k"},{fl:"ETA",fv:"2030"}]}
      ].map((k,i)=>(<DG key={i} accent={k.ac}>
        <Fab/>
        <Label>{k.l}</Label>
        <div style={{display:"flex",alignItems:"baseline",gap:4,marginTop:2}}>
          {k.u && <Unit>{k.u}</Unit>}
          <Hero>{k.v}</Hero>
          <Delta v={k.d} positive={k.pos}/>
        </div>
        <Sub>{k.sub}</Sub>
        {/* Family sub-tiles */}
        <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:8,marginTop:14,
          background:"rgba(255,255,255,0.03)",borderRadius:12,padding:"10px 12px"}}>
          {k.family.map((f,fi)=>(<div key={fi} style={{textAlign:"center"}}>
            <div style={{fontFamily:SANS,fontSize:9,color:"rgba(255,255,255,0.40)",letterSpacing:"0.04em",marginBottom:2}}>{f.fl}</div>
            <div style={{fontFamily:MONO,fontSize:14,fontWeight:700,color:"rgba(255,255,255,0.85)"}}>{f.fv}</div>
          </div>))}
        </div>
        <div style={{position:"absolute",bottom:0,left:24,right:24,height:40,opacity:0.6}}>
          <Spark data={k.sp} color={k.sc} w="100%" h={40}/>
        </div>
      </DG>))}
    </div>

    {/* ZONE 3: WIDE ACCENT STRIP \u2014 Portfolio Composition */}
    <DG span={12} accent={ACC} style={{background:"linear-gradient(135deg, rgba(212,168,67,0.08) 0%, rgba(212,168,67,0.02) 100%)",
      border:"1px solid rgba(212,168,67,0.12)"}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
        <Title>Portfolio Composition</Title>
        <span style={{fontFamily:MONO,fontSize:13,color:"rgba(255,255,255,0.55)"}}>\u00A3262,622 total</span>
      </div>
      <div style={{display:"flex",gap:4,height:36,borderRadius:10,overflow:"hidden"}}>
        {allocD.map((d,i)=>(<div key={i} style={{flex:d.value,background:d.color,display:"flex",alignItems:"center",justifyContent:"center",
          transition:"flex 0.3s ease",minWidth:d.value>5?0:2}}>
          {d.value>8 && <span style={{fontFamily:MONO,fontSize:9,fontWeight:600,color:"#fff",textShadow:"0 1px 3px rgba(0,0,0,0.4)"}}>{d.name} {d.value}%</span>}
        </div>))}
      </div>
      <div style={{display:"flex",gap:16,marginTop:10,flexWrap:"wrap"}}>
        {allocD.map((d,i)=>(<div key={i} style={{display:"flex",alignItems:"center",gap:5}}>
          <Dot c={d.color} sz={6}/>
          <span style={{fontFamily:SANS,fontSize:11,color:"rgba(255,255,255,0.55)"}}>{d.name}</span>
          <span style={{fontFamily:MONO,fontSize:11,fontWeight:600,color:"rgba(255,255,255,0.80)"}}>{d.value}%</span>
        </div>))}
      </div>
    </DG>

    {/* ZONE 4: NW TRAJECTORY (2fr) + SIDEBAR (1fr) */}
    <div style={{display:"grid",gridTemplateColumns:"2fr 1fr",gap:16}}>
      <DG accent={ACC}>
        <Fab icon={<Download size={12} color="rgba(255,255,255,0.45)"/>}/>
        <Title>Net Worth \u2014 Asset Flow Trajectory</Title>
        <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:12,marginTop:12}}>
          {[{l:"Weekly",v:"\u00A3262k",d:"+\u00A34k",p:true},{l:"Monthly",v:"\u00A3262k",d:"-\u00A31k",p:false},{l:"6M",v:"\u00A3262k",d:"-\u00A335k",p:false}].map((m,i)=>(<div key={i}>
            <div style={{fontFamily:SANS,fontSize:10,color:"rgba(255,255,255,0.45)"}}>{m.l}</div>
            <div style={{display:"flex",alignItems:"baseline",gap:4}}>
              <span style={{fontFamily:MONO,fontSize:24,fontWeight:700,color:"#fff"}}>{m.v}</span>
              <Delta v={m.d} positive={m.p}/>
            </div>
          </div>))}
        </div>
        <ChartBox>
        <ResponsiveContainer width="100%" height={260}>
          <AreaChart data={edgeD}>
            <defs>
              {[["t1d_pen","#7C6FFF",0.5],["t1d_eq","#0D9488",0.45],["t1d_cr",ACC,0.42],["t1d_ca","#34D399",0.38],["t1d_zar","#EF4444",0.35]].map(([id,c,o])=>(
                <linearGradient key={id} id={id} x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor={c} stopOpacity={o}/><stop offset="100%" stopColor={c} stopOpacity={0.05}/></linearGradient>
              ))}
            </defs>
            <CartesianGrid strokeOpacity={0.06} stroke="rgba(255,255,255,0.06)" strokeDasharray="3 3"/>
            <XAxis dataKey="m" tick={{fontSize:10,fill:"rgba(255,255,255,0.45)",fontFamily:MONO}} axisLine={false} tickLine={false}/>
            <YAxis tick={{fontSize:10,fill:"rgba(255,255,255,0.45)",fontFamily:MONO}} axisLine={false} tickLine={false} tickFormatter={v=>"\u00A3"+v+"k"} domain={[0,70]}/>
            <Area type="basis" dataKey="pension" stroke="#7C6FFF" fill="url(#t1d_pen)" strokeWidth={2} name="Pension" label={({x,y,value,index})=>index===0||index===6?<PillLabel x={x} y={y} value={"\u00A3"+value+"k"} color="#7C6FFF"/>:null}/>
            <Area type="basis" dataKey="equity" stroke="#0D9488" fill="url(#t1d_eq)" strokeWidth={2} name="Equities"/>
            <Area type="basis" dataKey="crypto" stroke={ACC} fill="url(#t1d_cr)" strokeWidth={2} name="Crypto"/>
            <Area type="basis" dataKey="cash" stroke="#34D399" fill="url(#t1d_ca)" strokeWidth={2} name="Cash"/>
            <Area type="basis" dataKey="zar" stroke="#EF4444" fill="url(#t1d_zar)" strokeWidth={2} name="ZAR"/>
          </AreaChart>
        </ResponsiveContainer>
        </ChartBox>
        <div style={{display:"flex",gap:14,marginTop:8}}>
          {[{n:"Pension",c:"#7C6FFF"},{n:"Equities",c:"#0D9488"},{n:"Crypto",c:ACC},{n:"Cash",c:"#34D399"},{n:"ZAR",c:"#EF4444"}].map((l,i)=>(<div key={i} style={{display:"flex",alignItems:"center",gap:4,fontSize:11,color:"rgba(255,255,255,0.55)",fontFamily:SANS}}><Dot c={l.c} sz={5}/>{l.n}</div>))}
        </div>
        <div style={{display:"flex",gap:16,marginTop:10}}>
          <Badge icon={<BarChart3 size={14} color="#22C55E"/>} label="Total assets" value="\u00A3375,670" color="#22C55E"/>
          <Badge icon={<TrendingDown size={14} color="#EF4444"/>} label="Total debts" value="\u00A313,048" color="#EF4444"/>
        </div>
      </DG>

      {/* Sidebar */}
      <div style={{display:"flex",flexDirection:"column",gap:16}}>
        <DG tier={1}><Fab/><Label>MODULE CONTROL</Label>
          {[["Return Mode",["TWR","XIRR","Real"],0],["Scenario",["Base","Bull","Bear"],0],["Benchmark",["60/40","MSCI","Custom"],0]].map(([l,opts,a],i)=>(<div key={i} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"6px 0"}}><span style={{fontFamily:SANS,fontSize:10,color:"rgba(255,255,255,0.48)"}}>{l}</span><Tog items={opts} a={a}/></div>))}
        </DG>
        <DG tier={1}><Fab icon={<AlertTriangle size={11} color="#F59E0B"/>}/><Label>RISK ALERTS</Label>
          {[{t:"CDaR\u2098 breached \u2014 BTC sleeve",s:"red",age:"2h"},{t:"Liquidity buffer below 5%",s:"amber",age:"1h"},{t:"Pension contribution on track",s:"green",age:"3h"}].map((a,i)=>(<div key={i} style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6}}><Pill text={a.t} sev={a.s}/><span style={{fontFamily:MONO,fontSize:8,color:"rgba(255,255,255,0.35)"}}>{a.age}</span></div>))}
        </DG>
        <DG tier={1}><Fab/><Label>THRESHOLD GOVERNANCE</Label>
          <div style={{display:"flex",justifyContent:"space-around",marginTop:6}}>
            {[{l:"Liquidity",v:17,c:"#EF4444"},{l:"Risk Budget",v:91,c:"#22C55E"},{l:"FIRE",v:34,c:ACC}].map((g,i)=>(<div key={i} style={{textAlign:"center"}}><div style={{width:48,height:48,borderRadius:"50%",border:"3px solid "+g.c,display:"flex",alignItems:"center",justifyContent:"center",margin:"0 auto 6px",background:g.c+"11",boxShadow:"0 0 14px "+g.c+"22"}}><span style={{fontFamily:MONO,fontSize:12,fontWeight:700,color:"#fff"}}>{g.v}%</span></div><div style={{fontFamily:SANS,fontSize:9,color:"rgba(255,255,255,0.45)"}}>{g.l}</div></div>))}
          </div>
        </DG>
        <DG tier={1}><Fab/><Label>ASSET ALLOCATION</Label>
          <div style={{display:"flex",alignItems:"center",gap:12,marginTop:4}}>
            ...
            <div style={{display:"flex",flexDirection:"column",gap:3}}>{allocD.map((d,i)=>(<div key={i} style={{display:"flex",alignItems:"center",gap:4,fontSize:9,fontFamily:SANS}}><Dot c={d.color} sz={5}/><span style={{color:"rgba(255,255,255,0.50)",minWidth:50}}>{d.name}</span><span style={{color:"rgba(255,255,255,0.85)",fontFamily:MONO,fontWeight:600}}>{d.value}%</span></div>))}</div>
          </div>
        </DG>
      </div>
    </div>

    {/* ZONE 5: QUALITY SCORE + DRAWDOWN (1fr 1fr) */}
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16}}>
      <DG accent="#7C6FFF"><Fab/>
        <Title>Quality Score</Title>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginTop:10}}>
          <div style={{flex:"0 0 100px"}}>
            <div style={{fontFamily:SANS,fontSize:10,color:"rgba(255,255,255,0.48)"}}><Dot c="#22C55E" sz={5}/> Current</div>
            <div style={{fontFamily:MONO,fontSize:32,fontWeight:700,color:"#fff",marginTop:4}}>5.2</div>
            <div style={{fontFamily:MONO,fontSize:12,fontWeight:600,color:"#22C55E",marginTop:3}}>+0.3</div>
            <div style={{fontFamily:SANS,fontSize:10,color:"rgba(255,255,255,0.40)"}}>vs 4.9 last quarter</div>
            <div style={{marginTop:10}}><Badge icon={<Shield size={11} color="#22C55E"/>} label="Risk quality" value="5.4/7" color="#22C55E"/></div>
          </div>
          <ResponsiveContainer width={180} height={170}>
            <RadarChart data={radarD}><PolarGrid stroke="rgba(255,255,255,0.08)"/><PolarAngleAxis dataKey="s" tick={{fontSize:8,fill:"rgba(255,255,255,0.45)"}}/><PolarRadiusAxis tick={false} axisLine={false} domain={[0,8]}/>
              <Radar dataKey="v" stroke="#22C55E" fill="#22C55E" fillOpacity={0.12} strokeWidth={2.5} style={{filter:"drop-shadow(0 0 6px rgba(34,197,94,0.35))"}}/>
              <Radar dataKey="t" stroke={ACC} fill="none" strokeWidth={1} strokeDasharray="4 4"/>
            </RadarChart>
          </ResponsiveContainer>
          <div style={{flex:"0 0 90px",textAlign:"right"}}>
            <div style={{fontFamily:SANS,fontSize:10,color:"rgba(255,255,255,0.48)"}}>Target</div>
            <div style={{fontFamily:MONO,fontSize:32,fontWeight:700,color:"#fff",marginTop:4}}>7.0</div>
            <div style={{fontFamily:SANS,fontSize:9,color:"rgba(255,255,255,0.35)",marginTop:3}}>Institutional benchmark</div>
            <div style={{marginTop:16}}><div style={{fontFamily:SANS,fontSize:10,color:"#EF4444"}}>Weakest</div><div style={{fontFamily:MONO,fontSize:22,fontWeight:700,color:"#EF4444"}}>3.8</div><div style={{fontFamily:SANS,fontSize:9,color:"rgba(255,255,255,0.35)"}}>Returns axis</div></div>
          </div>
        </div>
        <div style={{marginTop:8}}>{[{l:"Diversity",v:"7.6 / 7.0"},{l:"Tax Efficiency",v:"6.0 / 7.0"},{l:"Returns",v:"3.8 / 7.0"}].map((r,i)=><SR key={i} l={r.l} v={r.v}/>)}</div>
      </DG>

      <DG accent="#EF4444"><Fab icon={<Download size={12} color="rgba(255,255,255,0.45)"/>}/>
        <Title>Drawdown Profile</Title>
        <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:10,marginTop:10}}>
          {[{l:"Current",v:"-16.7%",c:"#EF4444"},{l:"Recovery",v:"In Progress",c:"rgba(255,255,255,0.55)"},{l:"Max DD",v:"-16.7%",c:"#EF4444"}].map((m,i)=>(<div key={i}>
            <div style={{fontFamily:SANS,fontSize:10,color:"rgba(255,255,255,0.45)"}}>{m.l}</div>
            <div style={{fontFamily:MONO,fontSize:20,fontWeight:700,color:m.c}}>{m.v}</div>
          </div>))}
        </div>
        <ChartBox>
        <ResponsiveContainer width="100%" height={150}>
          <AreaChart data={ddS.map((d,i)=>({x:["Sep","Oct","Nov","Dec","Jan","Feb","Mar","","","","",""][i]||("W"+i),v:d.v}))}>
            <defs><linearGradient id="t1d_dd" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#EF4444" stopOpacity={0.40}/><stop offset="100%" stopColor="#EF4444" stopOpacity={0.03}/></linearGradient></defs>
            <CartesianGrid strokeOpacity={0.06} stroke="rgba(255,255,255,0.06)" strokeDasharray="3 3"/>
            <XAxis dataKey="x" tick={{fontSize:9,fill:"rgba(255,255,255,0.45)"}} axisLine={false} tickLine={false}/>
            <YAxis tick={{fontSize:9,fill:"rgba(255,255,255,0.45)"}} axisLine={false} tickLine={false} tickFormatter={v=>v+"%"}/>
            <ReferenceLine y={-16.7} stroke="#EF4444" strokeDasharray="6 4" strokeWidth={1} strokeOpacity={0.5}/>
            <Area type="monotone" dataKey="v" stroke="#EF4444" fill="url(#t1d_dd)" strokeWidth={2.5} dot={false} style={{filter:"drop-shadow(0 0 6px rgba(239,68,68,0.4))"}}/>
          </AreaChart>
        </ResponsiveContainer>
        </ChartBox>
        <div style={{marginTop:6}}>{[{l:"Peak-to-Trough",v:"-\u00A349,834"},{l:"Recovery Needed",v:"+\u00A335,400"},{l:"Days in Drawdown",v:"142 days"}].map((r,i)=><SR key={i} l={r.l} v={r.v}/>)}</div>
      </DG>
    </div>

    {/* ZONE 6: UP/DOWN CAPTURE + BRINSON (1fr 1fr) */}
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16}}>
      <DG><Fab/>
        <Title>Up/Down Capture</Title>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginTop:10}}>
          <div><div style={{fontFamily:SANS,fontSize:10,color:"rgba(255,255,255,0.45)"}}><Dot c="#3B9EFF" sz={5}/> Up Capture</div><div style={{fontFamily:MONO,fontSize:24,fontWeight:700,color:"#fff"}}>68%</div></div>
          <div><div style={{fontFamily:SANS,fontSize:10,color:"rgba(255,255,255,0.45)"}}><Dot c="#EF4444" sz={5}/> Down Capture</div><div style={{fontFamily:MONO,fontSize:24,fontWeight:700,color:"#fff"}}>112%</div></div>
        </div>
        <ChartBox>
        <ResponsiveContainer width="100%" height={155}>
          <AreaChart data={captureD}>
            <defs>
              <linearGradient id="t1d_up" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#3B9EFF" stopOpacity={0.45}/><stop offset="100%" stopColor="#3B9EFF" stopOpacity={0.03}/></linearGradient>
              <linearGradient id="t1d_dn" x1="0" y1="1" x2="0" y2="0"><stop offset="0%" stopColor="#EF4444" stopOpacity={0.45}/><stop offset="100%" stopColor="#EF4444" stopOpacity={0.03}/></linearGradient>
            </defs>
            <CartesianGrid strokeOpacity={0.06} stroke="rgba(255,255,255,0.06)" strokeDasharray="3 3"/>
            <XAxis dataKey="m" tick={{fontSize:9,fill:"rgba(255,255,255,0.45)"}} axisLine={false} tickLine={false}/>
            <YAxis tick={{fontSize:9,fill:"rgba(255,255,255,0.45)"}} axisLine={false} tickLine={false}/>
            <ReferenceLine y={0} stroke="rgba(255,255,255,0.08)"/>
            <Area type="monotone" dataKey="up" stroke="#3B9EFF" fill="url(#t1d_up)" strokeWidth={2.5} dot={false}/>
            <Area type="monotone" dataKey="down" stroke="#EF4444" fill="url(#t1d_dn)" strokeWidth={2.5} dot={false}/>
          </AreaChart>
        </ResponsiveContainer>
        </ChartBox>
        <div style={{marginTop:6}}>{[{l:"Net Capture Ratio",v:"0.61x"},{l:"Asymmetry Score",v:"-0.44"},{l:"Batting Average",v:"42%"}].map((r,i)=><SR key={i} l={r.l} v={r.v}/>)}</div>
      </DG>
      <DG><Fab/>
        <Title>Brinson Attribution</Title>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginTop:10}}>
          <div><div style={{fontFamily:SANS,fontSize:10,color:"rgba(255,255,255,0.45)"}}>Allocation</div><div style={{fontFamily:MONO,fontSize:24,fontWeight:700,color:"#22C55E"}}>+2.1%</div></div>
          <div><div style={{fontFamily:SANS,fontSize:10,color:"rgba(255,255,255,0.45)"}}>Selection</div><div style={{fontFamily:MONO,fontSize:24,fontWeight:700,color:"#EF4444"}}>-9.8%</div></div>
        </div>
        <ChartBox>
        <ResponsiveContainer width="100%" height={170}>
          <BarChart data={contribD} layout="vertical" barCategoryGap="18%">
            <CartesianGrid strokeOpacity={0.06} stroke="rgba(255,255,255,0.06)" strokeDasharray="3 3"/>
            <XAxis type="number" tick={{fontSize:9,fill:"rgba(255,255,255,0.45)"}} axisLine={false} tickLine={false} tickFormatter={v=>(v>0?"+":"")+v+"k"}/>
            <YAxis type="category" dataKey="n" tick={{fontSize:10,fill:"rgba(255,255,255,0.55)"}} axisLine={false} tickLine={false} width={55}/>
            <ReferenceLine x={0} stroke="rgba(255,255,255,0.08)"/>
            <Bar dataKey="v" radius={[0,6,6,0]} barSize={14} label={{position:"right",fontSize:9,fill:"rgba(255,255,255,0.55)",fontFamily:MONO,formatter:v=>(v>0?"+":"")+v+"k"}}>{contribD.map((d,i)=><Cell key={i} fill={d.v>=0?"#22C55E":"#EF4444"} fillOpacity={0.75}/>)}</Bar>
          </BarChart>
        </ResponsiveContainer>
        </ChartBox>
        <Badge icon={<TrendingDown size={12} color="#EF4444"/>} label="Total active return" value="-7.6%" color="#EF4444"/>
      </DG>
    </div>

    {/* ZONE 7: RISK MAP + THERMAL HEATMAP (1fr 1fr) */}
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16}}>
      <DG><Fab/>
        <Title>Risk / Return Map</Title>
        <ChartBox>
        <ResponsiveContainer width="100%" height={200}>
          <ScatterChart><CartesianGrid strokeOpacity={0.06} stroke="rgba(255,255,255,0.06)" strokeDasharray="3 3"/>
            <XAxis type="number" dataKey="x" tick={{fontSize:9,fill:"rgba(255,255,255,0.45)"}} axisLine={false} tickLine={false} name="Vol %"/>
            <YAxis type="number" dataKey="y" tick={{fontSize:9,fill:"rgba(255,255,255,0.45)"}} axisLine={false} tickLine={false} name="Return %"/>
            <ReferenceLine y={0} stroke="rgba(255,255,255,0.06)"/>
            {riskRetD.map((b,i)=>(<Scatter key={i} data={[b]} fill={b.c} fillOpacity={0.65}><Cell r={Math.sqrt(b.z)*1.8}/></Scatter>))}
          </ScatterChart>
        </ResponsiveContainer>
        </ChartBox>
        <div style={{marginTop:6}}>{riskRetD.map((b,i)=>(<SR key={i} l={b.n} v={"Vol: "+b.x+"% \u00B7 Ret: "+(b.y>0?"+":"")+b.y+"%"}/>))}</div>
      </DG>
      <DG><Fab/>
        <Title>Monthly Returns Heatmap</Title>
        <ChartBox>
        <div style={{display:"grid",gridTemplateColumns:"55px repeat(6,1fr)",gap:3,fontSize:8,fontFamily:MONO,marginTop:4}}>
          <div/>{thermalD.months.map((m,i)=>(<div key={i} style={{textAlign:"center",color:"rgba(255,255,255,0.45)",padding:3}}>{m}</div>))}
          {thermalD.assets.map((asset,ri)=>(<Fragment key={ri}>
            <div style={{color:"rgba(255,255,255,0.55)",display:"flex",alignItems:"center",fontSize:9}}>{asset}</div>
            {thermalD.grid[ri].map((v,ci)=>{const c=v>2?"#22C55E":v>0?"#34D399":v>-2?ACC:v>-5?"#EF4444":"#DC2626";return(
              <div key={ci} style={{background:c+(Math.min(Math.round(Math.abs(v)*10+15),65).toString(16).padStart(2,"0")),borderRadius:6,padding:"6px 2px",textAlign:"center",color:"rgba(255,255,255,0.85)",fontWeight:600,fontSize:9}}>{v>0?"+":""}{v.toFixed(1)}</div>
            );})}
          </Fragment>))}
        </div>
        </ChartBox>
        <div style={{marginTop:6}}>{[{l:"Best Month",v:"Jan +1.2%"},{l:"Worst Month",v:"Nov -4.5%"},{l:"Hit Rate",v:"33% positive"}].map((r,i)=><SR key={i} l={r.l} v={r.v}/>)}</div>
      </DG>
    </div>

    {/* ZONE 8: MONTHLY BARS + LIQUIDITY (1fr 1fr) */}
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16}}>
      <DG><Fab/>
        <Title>Monthly Return Distribution</Title>
        <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:10,marginTop:10}}>
          <div><div style={{fontFamily:SANS,fontSize:10,color:"rgba(255,255,255,0.45)"}}>Positive</div><div style={{fontFamily:MONO,fontSize:24,fontWeight:700,color:"#22C55E"}}>2</div></div>
          <div><div style={{fontFamily:SANS,fontSize:10,color:"rgba(255,255,255,0.45)"}}>Negative</div><div style={{fontFamily:MONO,fontSize:24,fontWeight:700,color:"#EF4444"}}>4</div></div>
          <div><div style={{fontFamily:SANS,fontSize:10,color:"rgba(255,255,255,0.45)"}}>Avg</div><div style={{fontFamily:MONO,fontSize:24,fontWeight:700,color:"#fff"}}>-1.6%</div></div>
        </div>
        <ChartBox>
        <ResponsiveContainer width="100%" height={160}>
          <BarChart data={monthlyR} barCategoryGap="20%">
            <CartesianGrid strokeOpacity={0.06} stroke="rgba(255,255,255,0.06)" strokeDasharray="3 3"/>
            <XAxis dataKey="m" tick={{fontSize:10,fill:"rgba(255,255,255,0.45)"}} axisLine={false} tickLine={false}/>
            <YAxis tick={{fontSize:9,fill:"rgba(255,255,255,0.45)"}} axisLine={false} tickLine={false} tickFormatter={v=>v+"%"}/>
            <ReferenceLine y={0} stroke="rgba(255,255,255,0.08)"/>
            <Bar dataKey="v" radius={[6,6,0,0]} barSize={28} label={{position:"top",fontSize:9,fill:"rgba(255,255,255,0.55)",fontFamily:MONO,formatter:v=>(v>0?"+":"")+v+"%"}}>{monthlyR.map((d,i)=><Cell key={i} fill={d.v>=0?"#22C55E":"#EF4444"} fillOpacity={0.75}/>)}</Bar>
          </BarChart>
        </ResponsiveContainer>
        </ChartBox>
      </DG>
      <DG><Fab/>
        <Title>Liquidity Ladder</Title>
        <ChartBox>
        <ResponsiveContainer width="100%" height={200}>
          <BarChart data={liqD} barCategoryGap="25%">
            <CartesianGrid strokeOpacity={0.06} stroke="rgba(255,255,255,0.06)" strokeDasharray="3 3"/>
            <XAxis dataKey="n" tick={{fontSize:9,fill:"rgba(255,255,255,0.45)"}} axisLine={false} tickLine={false}/>
            <YAxis tick={{fontSize:9,fill:"rgba(255,255,255,0.45)"}} axisLine={false} tickLine={false} tickFormatter={v=>"\u00A3"+v+"k"}/>
            <Bar dataKey="v" radius={[6,6,0,0]} barSize={28} fill={ACC} fillOpacity={0.65} label={{position:"top",fontSize:9,fill:"rgba(255,255,255,0.55)",fontFamily:MONO,formatter:v=>"\u00A3"+v+"k"}}/>
          </BarChart>
        </ResponsiveContainer>
        </ChartBox>
        <div style={{marginTop:6}}>{[{l:"T+1 Liquid",v:"\u00A312k (4.6%)"},{l:"T+7 Available",v:"\u00A335k (13.3%)"},{l:"Illiquid",v:"\u00A322k (8.4%)"}].map((r,i)=><SR key={i} l={r.l} v={r.v}/>)}</div>
      </DG>
    </div>

    {/* ZONE 9: SCENARIO FIRE (full width) */}
    <DG accent={ACC}><Fab icon={<Star size={12} color={ACC}/>}/>
      <Title>FIRE Scenario Projection</Title>
      <ChartBox>
      <ResponsiveContainer width="100%" height={220}>
        <AreaChart data={scenD}>
          <defs>
            <linearGradient id="t1d_sc1" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#22C55E" stopOpacity={0.35}/><stop offset="100%" stopColor="#22C55E" stopOpacity={0.03}/></linearGradient>
            <linearGradient id="t1d_sc2" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#3B9EFF" stopOpacity={0.30}/><stop offset="100%" stopColor="#3B9EFF" stopOpacity={0.03}/></linearGradient>
          </defs>
          <CartesianGrid strokeOpacity={0.06} stroke="rgba(255,255,255,0.06)" strokeDasharray="3 3"/>
          <XAxis dataKey="y" tick={{fontSize:10,fill:"rgba(255,255,255,0.45)"}} axisLine={false} tickLine={false}/>
          <YAxis tick={{fontSize:10,fill:"rgba(255,255,255,0.45)"}} axisLine={false} tickLine={false} tickFormatter={v=>"\u00A3"+v+"k"}/>
          <Area type="monotone" dataKey="bull" stroke="#22C55E" fill="url(#t1d_sc1)" strokeWidth={2.5} dot={false} label={({x,y,value,index})=>index===4?<PillLabel x={x} y={y} value={"\u00A3"+value+"k"} color="#22C55E"/>:null}/>
          <Area type="monotone" dataKey="base" stroke="#3B9EFF" fill="url(#t1d_sc2)" strokeWidth={2} dot={false} label={({x,y,value,index})=>index===4?<PillLabel x={x} y={y} value={"\u00A3"+value+"k"} color="#3B9EFF"/>:null}/>
          <Line type="monotone" dataKey="bear" stroke="#EF4444" strokeWidth={2} dot={false} strokeDasharray="6 3" label={({x,y,value,index})=>index===4?<PillLabel x={x} y={y} value={"\u00A3"+value+"k"} color="#EF4444"/>:null}/>
          <ReferenceLine y={750} stroke={ACC} strokeDasharray="8 4" strokeOpacity={0.3}/>
        </AreaChart>
      </ResponsiveContainer>
      </ChartBox>
      <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:12,marginTop:10}}>
        {[{l:"FIRE Target",v:"\u00A3750k"},{l:"Coast FIRE (Base 2030)",v:"\u00A3398k"},{l:"Current Gap",v:"-\u00A3488k (-65%)"}].map((r,i)=>(<div key={i}><div style={{fontFamily:SANS,fontSize:10,color:"rgba(255,255,255,0.45)"}}>{r.l}</div><div style={{fontFamily:MONO,fontSize:16,fontWeight:700,color:"rgba(255,255,255,0.90)"}}>{r.v}</div></div>))}
      </div>
    </DG>

    {/* ZONE 10: HOLDINGS + KEY METRICS + SMALL KPIs (1fr 1fr 1fr) */}
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:16}}>
      <DG><Fab/>
        <Title>Top Holdings</Title>
        <table style={{width:"100%",borderCollapse:"collapse",marginTop:8}}>
          <thead><tr>{["Holding","Value","Wt","Ret","Ctb"].map((h,i)=>(<th key={i} style={{textAlign:i===0?"left":"right",fontFamily:SANS,fontSize:9,fontWeight:600,color:"rgba(255,255,255,0.40)",textTransform:"uppercase",letterSpacing:"0.06em",padding:"6px 4px",borderBottom:"1px solid rgba(255,255,255,0.08)"}}>{h}</th>))}</tr></thead>
          <tbody>{holdingsAll.map((h,i)=>(<tr key={i} style={{borderBottom:"1px solid rgba(255,255,255,0.04)",background:i%2===0?"transparent":"rgba(255,255,255,0.02)"}}><td style={{padding:"5px 4px",fontFamily:SANS,fontSize:11,color:"rgba(255,255,255,0.70)"}}>{h.n}</td><td style={{padding:"5px 4px",fontFamily:MONO,fontSize:11,color:"rgba(255,255,255,0.90)",textAlign:"right"}}>{h.v}</td><td style={{padding:"5px 4px",fontFamily:MONO,fontSize:11,color:"rgba(255,255,255,0.55)",textAlign:"right"}}>{h.w}</td><td style={{padding:"5px 4px",fontFamily:MONO,fontSize:11,fontWeight:600,color:h.c,textAlign:"right"}}>{h.r}</td><td style={{padding:"5px 4px",fontFamily:MONO,fontSize:11,color:h.c,textAlign:"right"}}>{h.cont}</td></tr>))}</tbody>
        </table>
        <Badge icon={<BarChart3 size={12} color="#22C55E"/>} label="Total positions" value="40 holdings" color="#22C55E"/>
      </DG>
      <DG><Fab/>
        <Title>Key Metrics</Title>
        {[["Sharpe Ratio","0.42","#EF4444"],["Sortino Ratio","0.31","#EF4444"],["Max Drawdown","-16.7%","#EF4444"],["HHI (Eff N)","5.8"],["Entropy","1.64"],["Active Share","72%",ACC],["Beta (vs MSCI)","0.87"],["Tracking Error","8.4%"]].map(([l,v,c],i)=><SR key={i} l={l} v={v} c={c}/>)}
        <Badge icon={<Activity size={12} color={ACC}/>} label="Overall quality" value="5.2 / 7.0" color={ACC}/>
      </DG>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
        {[{l:"SAVINGS RATE",v:"38%",d:"+2pp",p:true},{l:"RUNWAY",v:"4.2mo",d:"\u25BC 1.8",p:false},{l:"DEBT RATIO",v:"0%",d:"Clear",p:true},{l:"ISA USED",v:"\u00A320k",d:"Max",p:true},{l:"PENSION YTD",v:"\u00A312.3k",d:"+\u00A38.3k",p:true},{l:"DQS",v:"72",d:"+5",p:true}].map((k,i)=>(<DG key={i} tier={3} style={{padding:14,textAlign:"center"}}>
          <div style={{fontFamily:SANS,fontSize:9,color:"rgba(255,255,255,0.40)",letterSpacing:"0.06em",marginBottom:4}}>{k.l}</div>
          <div style={{fontFamily:MONO,fontSize:22,fontWeight:700,color:"#fff"}}>{k.v}</div>
          <div style={{marginTop:4}}><Delta v={k.d} positive={k.p}/></div>
        </DG>))}
      </div>
    </div>

    {/* ZONE 11: CAPITAL FLOW + WRAPPER (1fr 1fr) */}
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16}}>
      <DG accent={ACC}><Fab/>
        <Title>Capital Flow</Title>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginTop:10}}>
          <div><div style={{fontFamily:SANS,fontSize:10,color:"rgba(255,255,255,0.45)"}}>Gross Income</div><div style={{fontFamily:MONO,fontSize:24,fontWeight:700,color:"#fff"}}>\u00A3340k</div></div>
          <div><div style={{fontFamily:SANS,fontSize:10,color:"rgba(255,255,255,0.45)"}}>Investable</div><div style={{fontFamily:MONO,fontSize:24,fontWeight:700,color:"#22C55E"}}>\u00A3128k</div></div>
        </div>
        {[{l:"Gross Income",v:"\u00A3340k",c:"#22C55E",w:"100%"},{l:"Tax & NI",v:"\u00A3140k",c:"#EF4444",w:"41%"},{l:"Expenses",v:"\u00A372k",c:ACC,w:"21%"},{l:"Investable",v:"\u00A3128k",c:"#22C55E",w:"38%"}].map((f,i)=>(<div key={i} style={{marginTop:8}}><div style={{display:"flex",justifyContent:"space-between",marginBottom:4}}><span style={{fontFamily:SANS,fontSize:11,color:"rgba(255,255,255,0.55)"}}>{f.l}</span><span style={{fontFamily:MONO,fontSize:11,fontWeight:600,color:"rgba(255,255,255,0.85)"}}>{f.v}</span></div><div style={{height:8,borderRadius:6,background:"rgba(255,255,255,0.06)",overflow:"hidden"}}><div style={{height:"100%",width:f.w,background:"linear-gradient(90deg,"+f.c+"88,"+f.c+")",borderRadius:6,boxShadow:"0 0 10px "+f.c+"33"}}/></div></div>))}
      </DG>
      <DG accent="#7C6FFF"><Fab/>
        <Title>Wrapper Efficiency</Title>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginTop:10}}>
          <div><div style={{fontFamily:SANS,fontSize:10,color:"rgba(255,255,255,0.45)"}}>Tax-Free</div><div style={{fontFamily:MONO,fontSize:24,fontWeight:700,color:"#22C55E"}}>43%</div></div>
          <div><div style={{fontFamily:SANS,fontSize:10,color:"rgba(255,255,255,0.45)"}}>Taxable</div><div style={{fontFamily:MONO,fontSize:24,fontWeight:700,color:"#EF4444"}}>57%</div></div>
        </div>
        {[{l:"ISA",v:"\u00A320.0k / \u00A320k",p:100,c:"#22C55E"},{l:"Pension",v:"\u00A312.3k / \u00A360k",p:20,c:"#7C6FFF"},{l:"GIA",v:"\u00A323.8k / \u00A350k",p:48,c:ACC}].map((w,i)=>(<div key={i} style={{marginTop:8}}><div style={{display:"flex",justifyContent:"space-between",marginBottom:4}}><span style={{fontFamily:SANS,fontSize:11,color:"rgba(255,255,255,0.55)"}}>{w.l}</span><span style={{fontFamily:MONO,fontSize:11,fontWeight:600,color:w.c}}>{w.v}</span></div><div style={{height:8,borderRadius:6,background:"rgba(255,255,255,0.06)"}}><div style={{height:"100%",width:w.p+"%",background:"linear-gradient(90deg,"+w.c+"88,"+w.c+")",borderRadius:6,boxShadow:"0 0 10px "+w.c+"33"}}/></div></div>))}
        <div style={{marginTop:8}}>{[{l:"Annual Tax Drag",v:"\u00A34-7.3k"},{l:"CGT Allowance",v:"\u00A33,000"},{l:"Marginal Rate",v:"45% + 2% NI"}].map((r,i)=><SR key={i} l={r.l} v={r.v}/>)}</div>
      </DG>
    </div>

    {/* ZONE 12: STRENGTHS / WEAKNESSES / ACTIONS (1fr 1fr 1fr) */}
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:16}}>
      {[
        {t:"Strengths",c:"#22C55E",icon:<TrendingUp size={13}/>,items:["JPM Enhanced suite all positive: JUKC +16%, JURE +8%, JGEP +8%","Pension revalued +26% (+\u00A316.8k) \u2014 largest single contributor","15.4 effective positions with HHI 0.065 \u2014 genuinely diversified","New comp (\u00A3340k gross) transforms savings engine completely"]},
        {t:"Weaknesses",c:"#EF4444",icon:<AlertTriangle size={13}/>,items:["Crypto lost \u00A338k \u2014 32% of risk from 13% of capital","Cash buffer halved from \u00A334k to \u00A316k. Now 2.6mo vs 3mo target","Amex debt \u00A310,652 at 22% APR \u2014 most expensive capital","47% in taxable GIA wrapper \u2014 est. 1.5-2.0% annual tax drag"]},
        {t:"Priority Actions",c:ACC,icon:<Zap size={13}/>,items:["Max ISA (\u00A320k) before 5 April \u2014 29 days left. Non-negotiable","Clear Amex (\u00A310,652) from bonus \u2014 guaranteed 22% return","Salary sacrifice \u00A31,250/mo into pension \u2014 60% effective band","Consolidate 18 micro-positions to \u226415. Every position must earn its place"]}
      ].map((sec,i)=>(<DG key={i} accent={sec.c} style={{background:"linear-gradient(135deg, "+sec.c+"0A 0%, "+sec.c+"03 100%)"}}>
        <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:10}}><div style={{color:sec.c}}>{sec.icon}</div><span style={{fontFamily:SANS,fontSize:12,fontWeight:600,color:sec.c,textTransform:"uppercase",letterSpacing:"0.04em"}}>{sec.t}</span></div>
        {sec.items.map((item,j)=>(<div key={j} style={{fontFamily:SANS,fontSize:11,color:"rgba(255,255,255,0.65)",lineHeight:1.65,padding:"5px 0",borderBottom:"1px solid rgba(255,255,255,0.04)"}}><span style={{color:sec.c,marginRight:5,fontWeight:700,fontFamily:MONO}}>{j+1}.</span>{item}</div>))}
      </DG>))}
    </div>

    {/* FOOTER */}
    <div style={{textAlign:"center",padding:"16px 0",borderTop:"1px solid rgba(255,255,255,0.06)"}}>
      <div style={{fontFamily:MONO,fontSize:9,color:"rgba(255,255,255,0.30)"}}>LifeStack OS \u00B7 Data as of 7 March 2026 \u00B7 Portfolio value \u00A3262,072 \u00B7 Not investment advice</div>
    </div>
