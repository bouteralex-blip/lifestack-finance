...
const T1=()=>{
  // ═══ LOCAL LIGHT-MODE PALETTE — T1 ONLY ═══
  const L = {
    t1: "#0F172A",
    t2: "#334155",
    t3: "#64748B",
  ...
  // ═══ APPLE LIQUID GLASS — T1 TILE STANDARD ═══
  const liquidGlass = {
    background: "linear-gradient(135deg, rgba(255,255,255,0.12) 0%, rgba(200,230,255,0.08) 100%)",
    backdropFilter: "blur(28px) saturate(1.9)",
    WebkitBackdropFilter: "blur(28px) saturate(1.9)",
    boxShadow: [
      "inset 4px 4px 12px rgba(255,255,255,0.20)",
  ...
  /* G = Liquid Glass tile — T1 light mode. NOT the global Glass component. */
  ...
  const HBadge=({icon,label,value,color})=>(<div style={{display:"flex",alignItems:"center",gap:8,padding:"5px 0"}}>
    ...
    <div><div style={{fontSize:9,color:L.t3}}>{label}</div><div style={{fontSize:13,fontWeight:700,color:L.t1,fontFamily:MONO}}>{value}</div></div>
  </div>);

  const HExport=()=>(<button style={{background:"rgba(0,0,0,0.04)",border:"1px solid rgba(0,0,0,0.10)",borderRadius:8,padding:"4px 10px",color:L.t2,fontSize:9,cursor:"pointer",display:"flex",alignItems:"center",gap:4,fontFamily:MONO}}><Download size={10}/>Export</button>);
  const Spark=({data,color=L.teal,w=72,h=26})=>(<ResponsiveContainer width={w} height={h}><LineChart data={data}><Line type="monotone" dataKey="v" stroke={color} strokeWidth={1.8} dot={false} style={{filter:"drop-shadow(0 0 4px "+color+"55)"}}/></LineChart></ResponsiveContainer>);
  const Tog=({items,a=0})=>(<div style={{display:"flex",gap:2}}>{items.map((t,i)=>(<span key={i} style={{fontSize:8,fontFamily:MONO,fontWeight:600,padding:"3px 8px",borderRadius:6,background:i===a?T.blue+"30":"rgba(0,0,0,0.04)",color:i===a?T.blue:L.t3,border:"1px solid "+(i===a?T.blue+"55":"rgba(0,0,0,0.08)")}}>{t}</span>))}</div>);
  const Pill=({text,sev})=>{const bg=sev==="red"?"rgba(220,38,38,0.12)":sev==="amber"?"rgba(217,119,6,0.12)":"rgba(13,115,119,0.12)";const fg=sev==="red"?"#991B1B":sev==="amber"?"#92400E":"#065F63";const bc=sev==="red"?L.coral:sev==="amber"?L.accent:L.teal;return(<span style={{fontSize:9,fontWeight:600,color:fg,background:bg,padding:"3px 10px",borderRadius:16,borderLeft:"3px solid "+bc,fontFamily:MONO}}>{text}</span>);};
  const SR=({l,v,c})=>(<div style={{display:"flex",justifyContent:"space-between",padding:"5px 0",borderBottom:"1px solid "+L.grid}}><span style={{fontSize:11,color:L.t2}}>{l}</span><span style={{fontSize:11,fontFamily:MONO,fontWeight:600,color:c||L.t1}}>{v}</span></div>);
  const PillLabel=({x,y,value,color=L.t1})=>{
    if(!x||!y)return null;
    const w=String(value).length*6.5+16;
    return(<g><rect x={x-w/2} y={y-15} width={w} height={18} rx={9} fill="rgba(255,255,255,0.75)" stroke={color} strokeWidth={0.5} strokeOpacity={0.25}/><text x={x} y={y-3} textAnchor="middle" fill={color} fontSize={9} fontFamily={MONO} fontWeight="600">{value}</text></g>);
  };
  ...
  const allocD=[{name:"ETFs",value:27.9,color:L.teal},{name:"Pension",value:23,color:T.violet},{name:"Cash/FD",value:16.4,color:"#38BDF8"},{name:"Crypto",value:13.6,color:L.accent},{name:"Investments",value:12.2,color:T.blue},{name:"Stocks",value:3.6,color:"#EC4899"},{name:"Mixed",value:3.3,color:L.coral}];
  const captureD=["Sep","Oct","Nov","Dec","Jan","Feb","Mar"].map((m,i)=>({m,up:[8,5,3,2,6,4,7][i],down:[-4,-8,-12,-6,-10,-14,-5][i]}));
  const riskRetD=[{x:4,y:8,z:60,n:"Pension",c:T.violet},{x:8,y:-12,z:22,n:"BTC",c:L.accent},{x:3,y:7,z:24,n:"JURE",c:L.teal},{x:5,y:5,z:17,n:"JGEP",c:"#38BDF8"},{x:6,y:21,z:20,n:"ZAR",c:"#EC4899"},{x:2,y:4,z:46,n:"FD",c:"#34D399"}];
  ...
  return(<div style={{position:"relative",minHeight:"100vh"}}>
    ...
    {/* ZONE 1: HEADER */}
    <div style={{display:"flex",alignItems:"center",gap:12}}><Tag t="COMMAND CENTER"/><span style={{fontSize:20,fontWeight:700,color:L.t1,letterSpacing:"-0.02em"}}>Executive Summary</span></div>

    {/* ZONE 2: 4x PRIMARY KPI CARDS */}
    <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:16}}>
      {[
        {l:"Net Worth",v:"\u00A3262,622",d:"\u25BC 11.7%",dc:L.coral,sp:nwS,sc:L.coral,sub:"Peak: \u00A3297,457 \u00B7 Sep 2025",avg:"Avg. score \u00A3278,340",ac:L.accent},
        ...
        <div style={{fontSize:8,color:L.t3,marginTop:6,fontStyle:"italic"}}>{k.avg}</div>
      </G>))}
    </div>

    {/* ZONE 3: GRADIENT EDGE HERO (2fr) + SIDEBAR (1fr) */}
    <div style={{display:"grid",gridTemplateColumns:"2fr 1fr",gap:16}}>
      <G accent={L.accent} glow>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:4}}>
          <HTitle t="Net Worth \u2014 Asset Flow Trajectory"/>
          <div style={{display:"flex",gap:6,alignItems:"center"}}><Tag t="GRADIENT EDGE" c={L.t3}/><Star size={16} color={L.teal}/></div>
        </div>
        <HKpi items={[
          {dot:L.teal,label:"Weekly",value:"\u00A3262k",delta:"+\u00A34k",deltaC:L.teal,sub:"Compared to \u00A3258k last week"},
        ...
        <ResponsiveContainer width="100%" height={260}>
          <AreaChart data={edgeD}>
            <defs>
              {[["t1_pen",T.violet,0.6],["t1_eq",L.teal,0.55],["t1_cr",L.accent,0.5],["t1_ca","#34D399",0.45],["t1_zar",L.coral,0.4]].map(([id,c,o])=>(
            ...
            <Area type="basis" dataKey="equity" stroke={L.teal} fill="url(#t1_eq)" strokeWidth={0} name="Equities" label={({x,y,value,index})=>index===3?<PillLabel x={x} y={y} value={"\u00A3"+value+"k"} color={L.teal}/>:null}/>
            <Area type="basis" dataKey="crypto" stroke={L.accent} fill="url(#t1_cr)" strokeWidth={0} name="Crypto" label={({x,y,value,index})=>index===0||index===6?<PillLabel x={x} y={y} value={"\u00A3"+value+"k"} color={L.accent}/>:null}/>
            ...
            <Area type="basis" dataKey="zar" stroke={L.coral} fill="url(#t1_zar)" strokeWidth={0} name="ZAR"/>
            <ReferenceLine x="Nov" stroke="rgba(0,0,0,0.06)" strokeWidth={6}/>
        ...
        <div style={{display:"flex",gap:16,marginTop:6}}>
          <HBadge icon={<BarChart3 size={14} color={L.teal}/>} label="Total assets" value="\u00A3375,670" color={L.teal}/>
          <HBadge icon={<TrendingDown size={14} color={L.coral}/>} label="Total debts" value="\u00A313,048" color={L.coral}/>
        </div>
      ...
      <div style={{display:"flex",flexDirection:"column",gap:16}}>
        <G><div style={{display:"flex",justifyContent:"space-between",marginBottom:8}}><span style={{fontSize:11,fontWeight:700,color:L.t1,textTransform:"uppercase"}}>Module Control</span><Tag t="CMP-0034" c={L.t3}/></div>
          {[["Return Mode",["TWR","XIRR","Real"],0],["Scenario",["Base","Bull","Bear"],0],["Benchmark",["60/40","MSCI","Custom"],0]].map(([l,opts,a],i)=>(<div key={i} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"5px 0"}}><span style={{fontSize:9,color:L.t3}}>{l}</span><Tog items={opts} a={a}/></div>))}
        </G>
        <G><div style={{display:"flex",justifyContent:"space-between",marginBottom:6}}><span style={{fontSize:11,fontWeight:700,color:L.t1,textTransform:"uppercase"}}>Risk Alerts</span><Tag t="CMP-0035" c={L.t3}/></div>
          {[{t:"CDaR\u2098 breached \u2014 BTC sleeve",s:"red",age:"2k"},{t:"Liquidity buffer below 5%",s:"amber",age:"1h"},{t:"Pension contribution on track",s:"green",age:"3h"}].map((a,i)=>(<div key={i} style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:5}}><Pill text={a.t} sev={a.s}/><span style={{fontSize:8,color:L.t3}}>{a.age}</span></div>))}
        </G>
        <G><span style={{fontSize:11,fontWeight:700,color:L.t1,textTransform:"uppercase",marginBottom:6,display:"block"}}>Threshold Governance</span>
          <div style={{display:"flex",justifyContent:"space-around"}}>
            {[{l:"Liquidity",v:17,c:L.coral},{l:"Risk Budget",v:91,c:L.teal},{l:"FIRE",v:34,c:L.accent}].map((g,i)=>(<div key={i} style={{textAlign:"center"}}><div style={{width:44,height:44,borderRadius:"50%",border:"3px solid "+g.c,display:"flex",alignItems:"center",justifyContent:"center",margin:"0 auto 4px",background:g.c+"11",boxShadow:"0 0 12px "+g.c+"25"}}><span style={{fontSize:11,fontWeight:700,color:L.t1,fontFamily:MONO}}>{g.v}%</span></div><div style={{fontSize:8,color:L.t3,fontFamily:MONO}}>{g.l}</div></div>))}
          </div>
        </G>
        <G><span style={{fontSize:11,fontWeight:700,color:L.t1,textTransform:"uppercase",marginBottom:6,display:"block"}}>Asset Allocation</span>
          <div style={{display:"flex",alignItems:"center",gap:10}}>
            ...
            <div style={{display:"flex",flexDirection:"column",gap:2}}>{allocD.map((d,i)=>(<div key={i} style={{display:"flex",alignItems:"center",gap:4,fontSize:8}}><Dot c={d.color} sz={5}/><span style={{color:L.t2,minWidth:50}}>{d.name}</span><span style={{color:L.t1,fontFamily:MONO,fontWeight:600}}>{d.value}%</span></div>))}</div>
          </div>
        </G>
      </div>
    </div>
    ...
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16}}>
      <G accent={T.violet}>
        <div style={{display:"flex",justifyContent:"space-between"}}><HTitle t="Quality Score"/><Star size={14} color={L.accent}/></div>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
          <div style={{flex:"0 0 100px"}}>
            <div style={{display:"flex",alignItems:"center",gap:4,fontSize:10,color:L.t2}}><Dot c={L.teal}/>Current</div>
            <Plate><span style={{fontSize:24,fontWeight:700,color:L.t1,fontFamily:MONO}}>5.2</span></Plate>
            <div style={{fontSize:10,color:L.teal,fontWeight:700,marginTop:3}}>+0.3</div>
            <div style={{fontSize:8,color:L.t3}}>vs 4.9 last quarter</div>
          ...
          <div style={{flex:"0 0 100px",textAlign:"right"}}>
            <div style={{display:"flex",alignItems:"center",gap:4,fontSize:10,color:L.t2,justifyContent:"flex-end"}}><Star size={10} color={L.accent}/>Target</div>
            <Plate><span style={{fontSize:24,fontWeight:700,color:L.t1,fontFamily:MONO}}>7.0</span></Plate>
            <div style={{fontSize:8,color:L.t3,marginTop:3}}>Institutional benchmark</div>
            <div style={{marginTop:14}}><div style={{fontSize:10,color:L.coral}}>Weakest</div><div style={{fontSize:20,fontWeight:700,color:L.coral,fontFamily:MONO}}>3.8</div><div style={{fontSize:8,color:L.t3}}>Returns axis</div></div>
          </div>
        </div>
      ...
      <G accent={L.coral}>
        <div style={{display:"flex",justifyContent:"space-between"}}><HTitle t="Drawdown Profile"/><div style={{display:"flex",gap:4}}><Tag t="CMP-0063" c={L.t3}/><HExport/></div></div>
        <HKpi items={[
          {dot:L.coral,label:"Current",value:"-16.7%",delta:"Feb 2026",deltaC:L.coral,sub:"CDaR\u2098: -18.4%"},
      ...
      <G>
        ...
        <div style={{display:"grid",gridTemplateColumns:"55px repeat(6,1fr)",gap:2,fontSize:7,fontFamily:MONO}}>
          ...
          {thermalD.assets.map((asset,ri)=>(<Fragment key={ri}>
            <div style={{color:L.t2,display:"flex",alignItems:"center",fontSize:8}}>{asset}</div>
            {thermalD.grid[ri].map((v,ci)=>{const c=v>2?L.teal:v>0?"#34D399":v>-2?L.accent:v>-5?L.coral:"#EF4444";return(
    ...
    <G accent={L.teal} glow>
      <div style={{display:"flex",justifyContent:"space-between"}}><HTitle t="5-Year Wealth Scenarios"/><div style={{display:"flex",gap:4}}><Tag t="CMP-0070" c={L.t3}/><HExport/></div></div>
      <HKpi items={[
        {dot:L.teal,label:"Bull 2030",value:"\u00A3510k",delta:"+94%",deltaC:L.teal,sub:"12% CAGR compounded"},
      ...
      <ResponsiveContainer width="100%" height={180}>
        <AreaChart data={scenD}>
          ...
          <Line type="monotone" dataKey="bear" stroke={L.coral} strokeWidth={2} dot={false} strokeDasharray="6 3" label={({x,y,value,index})=>index===4?<PillLabel x={x} y={y} value={"\u00A3"+value+"k"} color={L.coral}/>:null}/>
    ...
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:16}}>
      ...
      <G>
        <div style={{display:"flex",justifyContent:"space-between"}}><HTitle t="Key Metrics"/><Tag t="ANALYTICS" c={L.t3}/></div>
        {[["Sharpe Ratio","0.42",L.coral],["Sortino Ratio","0.31",L.coral],["Max Drawdown","-16.7%",L.coral],["HHI (Eff N)","5.8"],["Entropy","1.64"],["Active Share","72%",L.accent],["Beta (vs MSCI)","0.87"],["Tracking Error","8.4%"]].map(([l,v,c],i)=><SR key={i} l={l} v={v} c={c}/>)}
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16}}>
        {[{l:"Savings Rate",v:"38%",d:"+2pp",c:L.teal,avg:"Avg. 32%"},{l:"Runway",v:"4.2mo",d:"\u25BC 1.8",c:L.coral,avg:"Target: 3.0mo"},{l:"Debt Ratio",v:"0%",d:"Clear",c:L.teal,avg:"Amex cleared"},{l:"ISA Used",v:"\u00A320k",d:"Max",c:L.teal,avg:"100% utilised"},{l:"Pension YTD",v:"\u00A312.3k",d:"+\u00A38.3k",c:L.teal,avg:"Target \u00A360k"},{l:"DQS",v:"72",d:"+5",c:L.accent,avg:"Decision Quality"}].map((k,i)=>(<G key={i} style={{...kpiGlass,padding:12,textAlign:"center"}}>
          ...
          <div style={{fontSize:7,color:L.t3,marginTop:2,fontStyle:"italic"}}>{k.avg}</div>
        </G>))}
      </div>
    </div>
    ...
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16}}>
      <G>
        <div style={{display:"flex",justifyContent:"space-between"}}><HTitle t="Capital Flow"/><Star size={14} color={L.accent}/></div>
        <HKpi items={[{dot:L.teal,label:"Gross Income",value:"\u00A3340k",sub:"Annual compensation"},{dot:L.teal,label:"Investable",value:"\u00A3128k",delta:"38%",deltaC:L.teal,sub:"Net deployable capital"}]}/>
        {[{l:"Gross Income",v:"\u00A3340k",c:L.teal,w:"100%"},{l:"Tax & NI",v:"\u00A3140k",c:L.coral,w:"41%"},{l:"Expenses",v:"\u00A372k",c:L.accent,w:"21%"},{l:"Investable",v:"\u00A3128k",c:L.teal,w:"38%"}].map((f,i)=>(<div key={i} style={{marginBottom:8}}><div style={{display:"flex",justifyContent:"space-between",marginBottom:3}}><span style={{fontSize:10,color:L.t2}}>{f.l}</span><span style={{fontSize:10,fontFamily:MONO,color:L.t1,fontWeight:600}}>{f.v}</span></div><div style={{height:7,borderRadius:4,background:"rgba(0,0,0,0.06)",overflow:"hidden"}}><div style={{height:"100%",width:f.w,background:"linear-gradient(90deg,"+f.c+"88,"+f.c+")",borderRadius:4,boxShadow:"0 0 8px "+f.c+"33"}}/></div></div>))}
        <HTable rows={[{city:"Savings Rate",values:["38%"]},{city:"Tax Rate (eff.)",values:["41%"]},{city:"Monthly Surplus",values:["\u00A310.7k"]}]}/>
      </G>
      <G>
        <div style={{display:"flex",justifyContent:"space-between"}}><HTitle t="Wrapper Efficiency"/><Tag t="TAX SHELTER" c={L.t3}/></div>
        <HKpi items={[{dot:L.teal,label:"Tax-Free",value:"43%",sub:"ISA + Pension wrapper"},{dot:L.coral,label:"Taxable",value:"57%",delta:"GIA dominant",deltaC:L.coral,sub:"Est. 1.5-2% drag"}]}/>
        {[{l:"ISA",v:"\u00A320.0k / \u00A320k",p:100,c:L.teal},{l:"Pension",v:"\u00A312.3k / \u00A360k",p:20,c:T.violet},{l:"GIA",v:"\u00A323.8k / \u00A350k",p:48,c:L.accent}].map((w,i)=>(<div key={i} style={{marginBottom:8}}><div style={{display:"flex",justifyContent:"space-between",marginBottom:3}}><span style={{fontSize:10,color:L.t2}}>{w.l}</span><span style={{fontSize:10,fontFamily:MONO,color:w.c,fontWeight:600}}>{w.v}</span></div><div style={{height:7,borderRadius:4,background:"rgba(0,0,0,0.06)"}}><div style={{height:"100%",width:w.p+"%",background:"linear-gradient(90deg,"+w.c+"88,"+w.c+")",borderRadius:4,boxShadow:"0 0 8px "+w.c+"33"}}/></div></div>))}
    ...
    {/* FOOTER */}
    <div style={{textAlign:"center",padding:"14px 0",borderTop:"1px solid rgba(0,0,0,0.08)"}}>
      <div style={{fontSize:8,color:L.t3,fontFamily:MONO}}>Data as of 7 March 2026. Portfolio value \u00A3262,072. Live state: unknown. Not investment advice.</div>
    </div>
