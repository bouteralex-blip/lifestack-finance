...
/**
 * LuminousStackedColumnWidget.jsx
 *
 * Stacked column chart with per-segment neon glow filters.
 *
 * Props:
 *   data     - Array of { label, segments: [{ key, value, color, glowColor? }] }
          ...
          {payload.map((p) => (
            <div key={p.dataKey} className="flex items-center justify-between gap-6">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs text-emerald-100/60">{p.dataKey}</span>
              </div>
...
export function LuminousStackedColumnWidget({
  data,
  title = "Stacked Breakdown",
  subtitle = "Luminous stacked column chart",
}) {
  const uid = useId();

  // Derive all unique segment keys and their colours from first row
  const allKeys = [];
  const colorMap = {};
  const glowMap = {};

  if (data?.length) {
    data[0].segments.forEach((seg) => {
      if (!allKeys.includes(seg.key)) {
        allKeys.push(seg.key);
        colorMap[seg.key] = seg.color;
        glowMap[seg.key] = seg.glowColor || seg.color;
      }
    });
  }

  // Flatten into Recharts-compatible rows
  const chartData = (data || []).map((row) => {
    const obj = { label: row.label };
    row.segments.forEach((seg) => {
      obj[seg.key] = seg.value;
    });
    return obj;
  });
              ...
              <BarChart data={chartData} margin={{ top: 10, right: 10, left: 0, bottom: 0 }} barCategoryGap="25%">
                ...
                {allKeys.map((key, ki) => (
                  <Bar
                    ...
                    stackId="s"
                    fill={colorMap[key]}
                    radius={ki === allKeys.length - 1 ? [4, 4, 0, 0] : [0, 0, 0, 0]}
                  >
                    {chartData.map((_, di) => (
                      <Cell
                        key={`${key}-${di}`}
                        style={{ filter: `drop-shadow(0 0 4px ${glowMap[key]}40)` }}
                      />
                    ))}
                  </Bar>
                ))}
          ...
          {allKeys.map((key) => (
            <div key={key} className="flex items-center gap-2">
              <div
                ...
                style={{ backgroundColor: colorMap[key], boxShadow: `0 0 6px ${glowMap[key]}40` }}
              />
              <span className="text-xs text-emerald-100/50">{key}</span>
            </div>
          ))}
        </div>
      </div>
...
export default LuminousStackedColumnWidget;
