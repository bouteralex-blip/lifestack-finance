    ...
    async function fetchAll() {
      try {
        const [configRes, holdingsRes, nwRes, bridgeRes, riskRes, cryptoRes, oppsRes,
               factorsRes, stressRes, bonusConfigRes, bonusScenariosRes, monthlyRes, scorecardRes] = await Promise.all([
        ...
        setData(mapped);
        setSource('supabase');
        console.log('LifeStack: Loaded live data from Supabase (14 tables) ✓');
      } catch (err) {
