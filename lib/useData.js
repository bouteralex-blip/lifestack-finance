...
function getIsaDeadlineFromReference(referenceIso) {
  ...
  const currentTaxYearDeadline = new Date(Date.UTC(year, 3, 5, 0, 0, 0));
  const deadline = baseDate > currentTaxYearDeadline ? new Date(Date.UTC(year + 1, 3, 5, 0, 0, 0)) : currentTaxYearDeadline;
  const daysRemaining = Math.max(0, Math.ceil((deadline.getTime() - baseDate.getTime()) / 86400000));
  return { deadline, daysRemaining };
}

function buildActionQueueState(mapped, sourceMeta) {
  if (!mapped?.PORT) return null;

  const isaAllowanceRemaining = 20000;
  const { deadline, daysRemaining: isaDeadlineDays } = getIsaDeadlineFromReference(sourceMeta?.snapshotDate);
  ...
  const actions = [
    {
      ...
      why: `${isaDeadlineDays} days to ${deadline.toISOString().slice(0, 10)}`,
...
function buildWatchlistState(mapped) {
  ...
  const movers = holdings
    ...
    .map(h => ({
      id: h.name,
      asset: h.name,
      changePct: Number((((h.val - h.prev) / h.prev) * 100).toFixed(1)),
      catalyst: h.cls === 'Crypto' ? 'Crypto volatility regime' : h.cls === 'ETF' ? 'Allocation drift / rotation' : 'Position-level review',
    }))
...
function buildTruthLayer(mapped, sourceMeta) {
  ...
  return {
    ...
    capital_efficiency_state: mapped?.SCORECARD
      ...
      : null,
    action_queue_state: buildActionQueueState(mapped, sourceMeta),
    watchlist_state: buildWatchlistState(mapped),
    decision_log: null,
