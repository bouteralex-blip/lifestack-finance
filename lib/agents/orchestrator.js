...
import { computePortfolioAdvisor } from './portfolio-advisor.js';
import { computeAlphaScanner } from './alpha-scanner.js';
import { computeRiskGuardian } from './risk-guardian.js';
import { computeCapitalDeployment } from './capital-deployment.js';
import { computeCIOMemo } from './cio-memo-writer.js';
...
function computeAllMarketEngines(rawData) {
  ...
  return {
    ...
    propertyCycle:       safeCompute('propertyCycle', computePropertyCycleState, md),
  };
}
...
function computeAllAgents(ENGINE, MKTENG, rawData) {
  ...
  return {
    ...
    // Phase 2.2 high-value agents
    portfolioAdvisor:    safeCompute('portfolioAdvisor', computePortfolioAdvisor, ENGINE, MKTENG, rawData),
    alphaScanner:        safeCompute('alphaScanner', computeAlphaScanner, ENGINE, MKTENG, rawData),
    riskGuardian:        safeCompute('riskGuardian', computeRiskGuardian, ENGINE, MKTENG, rawData),
    capitalDeployment:   safeCompute('capitalDeployment', computeCapitalDeployment, ENGINE, MKTENG, rawData),
  };
}
...
export function runWeeklyOrchestration(rawData) {
  ...
  const cioMemo   = safeCompute('cioMemo', computeCIOMemo, dailyResult.ENGINE, dailyResult.MKTENG, dailyResult.AGENT, rawData);

  return {
    ...dailyResult,
    weekly: { synthesis, review, memo, cioMemo },
  };
}
